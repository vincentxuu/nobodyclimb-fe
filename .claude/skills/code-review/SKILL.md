...
3. 如果 base branch 不是 main（例如 develop），自行判斷正確的 base
...
- **安全問題**：XSS、硬編碼 secret、不安全的 API 呼叫、JWT 處理
- **效能問題**：不必要的 re-render、D1 query 效能、KV/R2 過度呼叫、bundle size
- **架構一致性**：是否遵守專案既有模式（Hono 路由、Next.js App Router、Expo 結構等）
