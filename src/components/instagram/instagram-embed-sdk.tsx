...
/**
 * Instagram 官方 SDK 嵌入元件
 *
 * 使用 Instagram Embed.js SDK 來顯示完整的原始貼文內容
 * 包含圖片、文字、按讚數、留言數等所有資訊
 *
 * @example
 * ```tsx
 * <InstagramEmbedSDK url="https://www.instagram.com/p/DQ0D25cE4Wa/" />
 * ```
 */
export default function InstagramEmbedSDK({
  url,
  captioned = true,
  className = ''
  ...
  const embedUrl = `${url}${url.endsWith('/') ? '' : '/'}?utm_source=ig_embed&utm_campaign=loading`

  return (
    <div ref={containerRef} className={`instagram-embed-sdk-container ${className}`}>
      <blockquote
