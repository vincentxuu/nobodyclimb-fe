...
interface Biography {
  ...
  frequent_locations: string | null       // 平常出沒的地方 (儲存格式：逗號分隔；輸入方式：標籤輸入)
  favorite_route_type: string | null      // 喜歡的路線型態 (可複選，逗號分隔)
...
| | frequent_locations | 平常出沒的地方 | 標籤輸入：輸入後按 Enter 新增，點擊 × 刪除 | 地理連結 |
| | favorite_route_type | 喜歡的路線型態 | 可複選：攀登方式(抱石/運動攀登/頂繩攀登/速度/傳攀)、地形型態(平板岩/垂直/外傾/屋簷/裂隙/稜線/壁面/煙囪)、動作風格(動態/跑酷/協調/靜態/技術/力量/耐力) | 找同好 |
...
| | climbing_style | 最吸引你的攀岩風格 | 抱石/運動攀登/頂繩攀登/傳統攀登等攀登方式，或是特定地形（裂隙/屋簷等），為何吸引你 | 風格偏好 |
