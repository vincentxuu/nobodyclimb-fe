...
攀岩性格測驗的結果在完成當下就定型了——但攀岩者的風格會隨著訓練、經驗累積而演變。一個一開始偏 Power 的攀岩者，可能在大量 slab 練習後逐漸轉向 Technique；一個 Steady 型的人可能在連續突破難度後展現出 Bold 特質。靜態性格無法反映這些真實變化，錯失了「你進化了！」這種高度引人入勝的再參與時刻。透過分析攀登紀錄自動調整性格軸向，讓測驗結果成為動態的、會成長的個人識別，同時引入 Ego Grade（Casey Elliott 研究）量化攀岩者的「自我認知差距」。
...
- 新增 `GET /api/v1/quiz/evolution/ego-grade` 端點：查詢用戶 Ego Grade
- 新增 D1 migration：`personality_evolution` 資料表（演化歷史）、`users` 表新增 `ego_grade` 欄位
...
- `quiz-evolution`：性格演化系統——分析攀登紀錄自動調整性格軸向（onsight/redpoint 比率 → Goal/Free、路線類型偏好 → Power/Technique、難度突破頻率 → Bold/Steady），含 Ego Grade 計算、演化歷史時間軸、進化通知、最低資料門檻（20 筆紀錄）、週排程計算。
...
- 新增 migration：`personality_evolution` 表（id, user_id, from_type, to_type, power_pct, goal_pct, bold_pct, ego_grade, trigger, calculated_at）、`users` 表新增 `ego_grade REAL`
