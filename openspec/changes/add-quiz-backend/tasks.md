...
- [ ] 1.1 建立 `backend/migrations/0069_quiz_system.sql`，包含：
  - `CREATE TABLE quiz_results`（id, user_id, personality_type, power_pct, goal_pct, bold_pct, grit_index, flow_index, answers, version, created_at, updated_at）
...
- [ ] 2.1 建立 `backend/src/routes/quiz.ts`，定義 Hono router 並匯出
- [ ] 2.2 實作 `POST /api/v1/quiz/results`：
...
- [ ] 2.3 實作 `GET /api/v1/quiz/results/me`：
  - Required auth middleware
  - 查詢 quiz_results WHERE user_id = ? ORDER BY created_at DESC
  - 回傳 `{ latest, history }`
- [ ] 2.4 實作 `GET /api/v1/quiz/stats`：
...
- [ ] 2.5 為所有端點加上 `hono-openapi` route decorator（summary、description、request/response schema）

## 3. Quiz Ranking API

- [ ] 3.1 實作 `GET /api/v1/quiz/ranking/:type`：
...
- [ ] 3.2 加上 OpenAPI decorator

## 4. Training API（backend/src/routes/training.ts）

- [ ] 4.1 建立 `backend/src/routes/training.ts`，定義 Hono router 並匯出
- [ ] 4.2 實作 `GET /api/v1/training/plan/:type`：
  - 驗證 `:type` 為合法代碼
  - 從 `@nobodyclimb/constants` import 訓練計畫定義
  - 回傳對應型態的 TrainingPlan
- [ ] 4.3 實作 `POST /api/v1/training/progress`：
  - Required auth middleware
  ...
  - 回傳 200
- [ ] 4.4 實作 `GET /api/v1/training/progress/me`：
  - Required auth middleware
  - 支援 `?type=PGB` query parameter 篩選
  - 查詢 training_progress WHERE user_id = ?（加 type 篩選如有）
  - 回傳進度列表
- [ ] 4.5 為所有端點加上 OpenAPI decorator

## 5. Climber Rank 積分修改

- [ ] 5.1 修改 `backend/src/services/rank.ts` 的 `calculateUserScore` 函式：
  - 新增查詢：`SELECT COUNT(*) FROM quiz_results WHERE user_id = ?`（有記錄 → +5 分）
  - 新增查詢：`SELECT personality_type, COUNT(*) as cnt FROM training_progress WHERE user_id = ? AND completed = 1 GROUP BY personality_type HAVING cnt = 12`（有完成型態 → +15 分，僅計一個）
- [ ] 5.2 擴充 `RankScoreBreakdown` 型別，新增 `quiz_completed` 與 `training_completed` 欄位
- [ ] 5.3 更新 breakdown 回傳值，包含新欄位

## 6. 路由註冊

- [ ] 6.1 修改 `backend/src/index.ts`：import 並註冊 quiz 和 training 路由

## 7. 驗證

- [ ] 7.1 本地啟動 backend（`pnpm dev`），測試所有端點回應格式正確
- [ ] 7.2 驗證 OpenAPI JSON（`/api/v1/openapi.json`）包含所有新端點
- [ ] 7.3 驗證 Scalar UI（`/api/v1/docs`）可正確顯示新端點文件
- [ ] 7.4 執行 `pnpm typecheck` 確認無型別錯誤
- [ ] 7.5 執行 `pnpm lint` 確認無 lint 錯誤
