...
export async function migrateJsonStories(db: D1Database): Promise<void> {
  ...
  let _coreStoriesInserted = 0
  let _oneLinersInserted = 0
  let _storiesInserted = 0
  ...
  for (const bio of biographies.results || []) {
    ...
    if (bio.stories_data) {
      try {
        const storiesData = JSON.parse(bio.stories_data) as Record<
          string,
  ...
  if (coreStoryStmts.length > 0) {
    ...
    _coreStoriesInserted = results.reduce((acc, res) => acc + (res.meta.changes || 0), 0)
  }
  if (oneLinerStmts.length > 0) {
    const results = await db.batch(oneLinerStmts)
    _oneLinersInserted = results.reduce((acc, res) => acc + (res.meta.changes || 0), 0)
  }
  if (storyStmts.length > 0) {
    const results = await db.batch(storyStmts)
    _storiesInserted = results.reduce((acc, res) => acc + (res.meta.changes || 0), 0)
  }
}
