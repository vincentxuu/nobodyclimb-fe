...
export class BiographyContentRepository {
  ...
  async batchGetStories(biographyIds: string[]): Promise<Map<string, StoriesData>> {
    ...
    const storiesResult = await this.db
      .prepare(query)
      .bind(...biographyIds)
      .all<{
        biography_id: string
    ...
    for (const [questionId, data] of Object.entries(oneLinersData)) {
      ...
      if (isCoreStory) {
        ...
        if (!answer || answer.trim() === '') {
          ...
          if (existing) {
            await this.db
              .prepare('DELETE FROM biography_core_stories WHERE id = ?')
              .bind(existing.id)
              .run()
          }
        } else if (existing) {
          // 更新現有記錄
          await this.db
            .prepare('UPDATE biography_core_stories SET content = ?, updated_at = ? WHERE id = ?')
            .bind(answer.trim(), now, existing.id)
            .run()
        } else {
          // 插入新記錄
          ...
          await this.db
            ...
            .bind(id, biographyId, questionId, answer.trim(), now, now)
            .run()
        }
      } else {
        // 處理 one-liners
        const existing = await this.db
          .prepare('SELECT id FROM biography_one_liners WHERE biography_id = ? AND question_id = ?')
          .bind(biographyId, questionId)
          .first<{ id: string }>()
        ...
        if (!answer || answer.trim() === '') {
          ...
          // 更新現有記錄
          await this.db
            .prepare('UPDATE biography_one_liners SET answer = ?, updated_at = ? WHERE id = ?')
            .bind(answer.trim(), now, existing.id)
            .run()
        } else {
          // 插入新記錄
  ...
  async syncStories(
    ...
    for (const [categoryId, questions] of Object.entries(storiesData)) {
      ...
      for (const [questionId, data] of Object.entries(questions)) {
        ...
        const existing = await this.db
          .prepare('SELECT id FROM biography_stories WHERE biography_id = ? AND question_id = ?')
          .bind(biographyId, questionId)
          .first<{ id: string }>()
