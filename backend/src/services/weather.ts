import { CwaLocationWeather, CwaWeatherElement, Env, WeatherData } from '../types'
...
function normalizeLocationData(locationData: any): CwaLocationWeather {
  ...
  const mapping = [
    ...
    {
      names: ['天氣預報綜合描述', 'WeatherDescription'],
    ...
    {
      names: ['12小時降雨機率', 'ProbabilityOfPrecipitation'],
      target: 'PoP12h',
      key: 'ProbabilityOfPrecipitation',
    },
    {
      names: ['最大舒適度指數', '最大舒適度指數描述', 'MaxComfortIndexDescription'],
      target: 'CI',
      key: 'MaxComfortIndexDescription',
    },
    {
      names: ['最小舒適度指數', '最小舒適度指數描述', 'MinComfortIndexDescription'],
      target: 'CI',
      key: 'MinComfortIndexDescription',
    },
  ]
  ...
  for (const map of mapping) {
    const element = weatherElement.find((entry: any) =>
      map.names.some(
        (name) =>
          normalizeElementName(entry.ElementName || entry.elementName || '') ===
...
export async function getWeatherByLocation(
  ...
  try {
    ...
    if (locations.length === 0) {
      ...
      if (city && district) {
        const fallbackData = await fetchCityForecast(
          env,
          datasetId,
          displayCity,
          district,
    ...
    if (district) {
      ...
      if (matched) {
        ...
        console.warn(
          `No district match found in city data: city="${displayCity}", district="${district}"`
        )
      }
    }
...
async function fetchTaiwanForecast(
  ...
  try {
    ...
    const weatherData = parseWeatherData(
      locationData,
      locationData.locationName || displayCity,
      null,
      originalLocation
    )
...
async function fetchCityForecast(
  ...
  try {
    ...
    if (!matchedLocation) {
      console.warn(
        `No district match found in city forecast: city="${displayCity}", district="${district}"`
      )
      return null
    }
