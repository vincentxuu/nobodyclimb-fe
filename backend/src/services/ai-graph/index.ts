...
type AnyGraph = {
  invoke(state: GraphState, config?: { recursionLimit?: number }): Promise<GraphState>
}
...
function extractPhaseLatency(trace: Record<string, unknown>): {
  ...
  const embeddingTrace = trace.embedding as { duration_ms?: number } | undefined
  const embeddingMs = embeddingTrace?.duration_ms ?? null

  // retrieval: 從 hybrid_search / agentic_retrieve 的 trace 提取
  const hybridTrace = trace.hybrid_search as { total_duration_ms?: number } | undefined
  const _agenticTrace = trace.agentic_retrieve as { step?: number } | undefined
...
async function postGraphProcessing(state: GraphState): Promise<AIAskResponse> {
  ...
  const totalStageTokens = sumTokenBreakdown(state.tokenBreakdown as Record<string, unknown>)
  ...
  const queryId = await queryService.logQuery({
    ...
    sources: state.request.include_sources !== false ? (state.sources ?? []) : [],
    ...
    pipelineTrace: Object.keys(finalTrace).length > 0 ? JSON.stringify(finalTrace) : undefined,
  })
