...
const SQL_LIST_PATTERN =
  /有哪些.*路線|列出.*路線|有什麼.*路線|路線有哪些|路線有什麼/;

function inferSqlQuery(
  query: string,
  cragNames: string[],
...
export async function toolSelectionNode(
  state: GraphState,
  ...
  try {
    ...
    const wantsRecommendation =
      /推薦|建議|類似|相似|差不多|下一條|下一個|suggest/i.test(query);
    const isPersonalQuery =
      PERSONAL_QUERY_PATTERN.test(query) && !wantsRecommendation;
    ...
    if (hasSimRouteIntent) {
      ...
      if (hydeDocResult.doc) {
        updates.trace = {
          ...updates.trace,
          hyde: { document: hydeDocResult.doc.slice(0, 300) },
        };
      }
      ...
      if (routeRefs.length > 0) {
        ...
        // 多岩場：收集所有 crag_id，單一用 $eq，多個用 $in
        const cragIds = [
          ...new Set(routeRefs.map((r) => r.cragId).filter(Boolean)),
        ] as string[];
        if (cragIds.length === 1) {
        ...
        const gradeNumerics = routeRefs
          .map((r) => r.gradeNumeric)
          .filter((g) => g > 0);
        if (gradeNumerics.length > 0) {
          const minGrade = Math.min(...gradeNumerics);
          const maxGrade = Math.max(...gradeNumerics);
          const minRange = state.queryService.similarGradeRange(minGrade, 3);
          const maxRange = state.queryService.similarGradeRange(maxGrade, 3);
          vectorFilter["grade_numeric"] = {
            $gte: minRange.$gte,
            $lte: maxRange.$lte,
          };
        }
        ...
        // 向後相容：保留第一條的 excludeRouteId
        updates.excludeRouteId = routeRefs[0].routeId;

        // referenceRouteInfo 列出所有參考路線
      ...
      endSpan(span, {
        output: {
          ...
          referenceRoutes: routeRefs.map((r) => r.name),
    ...
    ).replace("{tools}", toolRegistry.generatePromptBlock());
    const { result: parsedQuery, usage: toolSelectionUsage } =
      await state.queryService.parseQueryWithLLM(
        query,
        llmModel,
        cragNames,
        areaNames,
    ...
    if (toolSelectionUsage) {
      updates.tokenBreakdown = {
        ...tokenBreakdown,
        tool_selection: { ...toolSelectionUsage, model: llmModel },
      };
    }
    ...
    if (parsedQuery) {
      ...
      newTrace["tool_selection"] = {
        ...
        confidence: parsedQuery.confidence ?? 1.0,
        ...(parsedQuery.alternative
          ? { alternative: parsedQuery.alternative }
          : {}),
    ...
    if (parsedQuery?.tool === "general_knowledge") {
      ...
      if (mt?.steps && Array.isArray(mt.steps) && mt.steps.length > 0) {
        ...
        if (validSteps.length > 0) {
          ...
          updates.multiToolPlan = {
            ...
            execution_mode:
              mt.execution_mode === "sequential" ? "sequential" : "parallel",
          };
          updates.effectiveLlmModel = llmModel;
        } else {
      ...
      if (isPersonalQuery) {
        ...
        updates.sqlParams = {};
        updates.effectiveLlmModel = pipelineConfig.lightweight_model;
        const qpTrace = newTrace["query_parsing"] as
      ...
      // 安全網 2：SQL 計數/清單查詢（需已知岩場名稱）
        ...
        if (sqlOverride) {
          ...
          updates.effectiveLlmModel =
            updates.queryType === "simple"
              ? pipelineConfig.simple_model
              : llmModel;
        }
      }
    }
    ...
    if (pipelineConfig.rag_strategy === "auto" && parsedQuery) {
      ...
      if (
        ...
        (VALID_HINTS as readonly string[]).includes(rawHint)
      ) {
    ...
    if (parsedQuery?.retrieval_method) {
      ...
      if (
        (VALID_METHODS as readonly string[]).includes(
          parsedQuery.retrieval_method,
        )
      ) {
    ...
    const traceObj = (updates.trace?.["query_parsing"] ??
      newTrace["query_parsing"]) as Record<string, unknown> | undefined;
    const regexUsed =
      traceObj?.personal_query_fallback || traceObj?.sql_query_fallback;
