...
export async function toolSelectionNode(state: GraphState): Promise<Partial<GraphState>> {
  ...
  try {
    ...
    // 動態注入工具描述到 prompt
    const toolSelectionPrompt = (prompts['TOOL_SELECTION_PROMPT'] || '').replace(
      '{tools}',
      ...
      // 安全網 2：SQL 計數/清單查詢（需已知岩場名稱）
        ...
        if (sqlOverride) {
          ...
          updates.effectiveLlmModel =
            updates.queryType === 'simple' ? pipelineConfig.simple_model : llmModel
        }
      }
    }
    ...
    const traceObj = (updates.trace?.['query_parsing'] ?? newTrace['query_parsing']) as
