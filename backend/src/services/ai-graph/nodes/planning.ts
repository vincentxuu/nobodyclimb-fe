...
export async function planningNode(state: GraphState): Promise<Partial<GraphState>> {
  ...
  try {
    const { request, pipelineConfig: cfg, preloadedCrags, preloadedAreas, prompts, llmProvider } = state;
    ...
    let rawText = '';
    try {
      const planPromise = llmProvider.chat(
        [{ role: 'user', content: prompt }],
        { model: cfg.llm_model, gatewayOptions: state.gatewayOptions },
      );
