...
export async function popularityRerankNode(state: GraphState): Promise<Partial<GraphState>> {
  ...
  try {
    ...
    // NOTE: climbed_route_ids 不在 GraphState 中，此欄位由 RecommendationService 注入 PipelineContext
    // 在 LangGraph 版本中暫不支援此功能
    ...
    const videoCountRecord: Record<string, number> = Object.fromEntries(videoCountMap);
    const latestVideoRecord: Record<string, string> = Object.fromEntries(latestVideoMap);
    ...
    // generation trace
    const isPersonalized = !!(state.memorySummary || state.ascentContext?.length);
    const generationTrace = {
      context_doc_count: orderedDocs.length,
      personalized: !!state.userId,
      regen_triggered: false,
      ability_level: state.abilityLevel,
      memory_summary_length: state.memorySummary ? state.memorySummary.length : 0,
      ...
      memory_summary_preview: state.memorySummary ? state.memorySummary.slice(0, 200) : null,
    };
