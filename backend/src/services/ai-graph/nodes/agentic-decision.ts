  ...
  try {
    ...
    if (loopCount >= pipelineConfig.agentic_max_steps) {
      ...
      return {
        ...
        trace: {
          agentic_decision: { step: loopCount, action: 'ANSWER', reason: 'max_steps_reached' },
        },
      }
    }
    ...
    if (currentDocs.length >= pipelineConfig.agentic_min_docs_to_answer) {
      endSpan(span, {
        output: { action: 'ANSWER', reason: 'enough_docs', docCount: currentDocs.length },
      })
      return {
        agenticAction: 'ANSWER',
        loopCount: loopCount + 1,
    ...
    if (state.isSimRouteSearch && loopCount === 0 && currentDocs.length === 0) {
      ...
      endSpan(span, {
        output: { action: 'RETRIEVE', reason: 'sim_route_first_loop', refinedQuery: simQuery },
      })
      return {
        ...
        trace: {
          agentic_decision: {
            step: loopCount,
            action: 'RETRIEVE',
            reason: 'sim_route_first_loop',
            refined_query: simQuery,
          },
        },
      }
    }
    ...
    if (!state.llmProvider) {
      ...
      return {
        ...
        loopCount: loopCount + 1,
        trace: {
          agentic_decision: { step: loopCount, action: fallbackAction, reason: 'no_llm_provider' },
        },
      }
    }
    ...
    try {
      const llmResult = await state.llmProvider.chat([{ role: 'user', content: prompt }], {
      ...
      if (jsonMatch) {
        ...
        if (
          parsed.type === 'RETRIEVE' &&
          typeof parsed.refinedQuery === 'string' &&
          parsed.refinedQuery.trim().length > 0
        ) {
          action = 'RETRIEVE'
    ...
    if (tokenUsage) {
      const existingDecisions =
        (newTokenBreakdown['agentic_decisions'] as
          | Array<{
