...
export async function llmGenerationNode(state: GraphState): Promise<Partial<GraphState>> {
  ...
  try {
    ...
    if (state.queryType === 'general-knowledge') {
      ...
      if (llmResult.usage) {
        newTokenBreakdown.main_generation = { ...llmResult.usage, model: effectiveLlmModel, estimated: false };
      } else {
        const estP = Math.ceil((gkPersonalized.length + query.length) / 2);
        const estC = Math.ceil(answer.length / 2);
        newTokenBreakdown.main_generation = { prompt_tokens: estP, completion_tokens: estC, total_tokens: estP + estC, model: effectiveLlmModel, estimated: true };
      }
      ...
      if (state.userId && state.waitUntilCtx) {
        ...
        memoryTrace = { triggered: false, async: false, reason: state.userId ? 'no_ctx' : 'anonymous' };
      }
      ...
      const queryId = await queryService.logQuery({
        ...
        tokenCount: totalTokens > 0 ? totalTokens : (llmResult.usage?.total_tokens ?? Math.ceil((gkPersonalized.length + query.length + answer.length) / 2)),
      ...
      await state.env.CACHE.put(state.cacheKey, JSON.stringify(gkResponse), { expirationTtl: state.cacheTtl });
      ...
      // 非同步記憶體萃取
      if (state.userId && state.waitUntilCtx) {
        const gkGatewayOpts = state.env.AI_GATEWAY_SLUG ? { gateway: { id: state.env.AI_GATEWAY_SLUG } } : undefined;
    ...
    // RAG 路徑（含 hybrid 分支）
    const context = state.queryType === 'hybrid' && state.sqlContext ? state.sqlContext : state.context ?? '目前沒有找到相關資料。';
    ...
    const newTokenBreakdown = { ...tokenBreakdown };
    if (llmUsage) {
      newTokenBreakdown.main_generation = { ...llmUsage, model: effectiveLlmModel, estimated: false };
    } else {
