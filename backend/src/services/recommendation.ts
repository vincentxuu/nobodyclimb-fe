import { AISource, Env } from '../types'
...
// 從完攀紀錄取最高難度，計算推薦目標範圍
export function getTargetGradeRange(
  ascents: RecentAscent[]
): { maxGrade: string; targetMin: string; targetMax: string } | null {
  const numerics = ascents.map((a) => gradeToNumeric(a.grade)).filter((n) => n > 0)
