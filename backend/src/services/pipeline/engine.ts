...
import { getDefaultStepConfigs, STEP_REGISTRY } from './registry'
...
function getStepTimeout(stepId: StepId, config: PipelineConfig, ctx?: PipelineContext): number {
  switch (stepId) {
    case 'hyde':
      return config.hyde_timeout_ms
    case 'multi-query':
      return config.multi_query_timeout_ms
    case 'embedding':
      return config.embedding_timeout_ms
    case 'hybrid-search': {
      // Plan-and-Execute 路徑需要更多時間（planning + execution + synthesis）
      const strategy =
        config.rag_strategy === 'auto' ? (ctx?.strategyHint ?? 'baseline') : config.rag_strategy
      if (strategy === 'plan-execute') {
        return (
          config.planning_timeout_ms +
          config.synthesis_timeout_ms +
    ...
    case 'llm-generation':
      return config.generation_timeout_ms
    case 'judge':
      return config.judge_timeout_ms
...
export class PipelineEngine {
    ...
    for (const config of enabledSteps) {
      ...
      for (const req of meta.requires) {
        if (!providedFields.has(req as string)) {
          errors.push(
            `Step "${meta.id}" requires "${String(req)}" but no preceding enabled step provides it`
          )
        }
      }
  ...
  private evaluateSkipWhen(
    ...
    for (const cond of conditions) {
      ...
      if (matches) {
        return {
          skip: true,
          reason: `skipWhen: ${String(cond.field)} ${cond.operator} ${String(cond.value)}`,
        }
      }
    }
    ...
    while (stepIdx < enabledSteps.length) {
      ...
      if (branchStepSet.has(config.id)) {
        ...
        if (bc) {
            ...
            for (const bStepId of branchSteps) {
              ...
              if (bStep) {
                try {
                  ...
                  ;(branchCtx.trace.step_errors as Array<{ step: string; error: string }>).push({
                    step: bStepId,
                    error: errorMsg,
                  })
                }
          ...
          while (
            stepIdx < enabledSteps.length &&
            allBranchStepIds.has(enabledSteps[stepIdx].config.id)
          ) {
            stepIdx++
          }
          continue
      ...
      try {
        ...
        ;(
          ctx.trace.step_errors as Array<{
            step: string
            error: string
            phase: string
            timeout?: boolean
      ...
      if (ctx.loopBack) {
        ...
        if (ctx.loopCount < maxLoops) {
          ...
          const jumpIdx = enabledSteps.findIndex(
            (s) => PHASE_ORDER.indexOf(s.meta.phase) >= targetPhaseIdx
          )
          if (jumpIdx >= 0) {
            stepIdx = jumpIdx
  ...
  private async postPipelineProcessing(ctx: PipelineContext): Promise<void> {
    ...
    const estimatedTokens = Math.ceil(
      ((ctx.prompts['SYSTEM_PROMPT']?.length ?? 0) +
    ...
    const tokenCount =
      totalStageTokens > 0 ? totalStageTokens : (mainGenUsage?.total_tokens ?? estimatedTokens)

    // memory extraction trace
    if (ctx.userId && ctx.waitUntilCtx) {
      ctx.trace.memory_extraction = { triggered: true, async: true }
    } else {
      ctx.trace.memory_extraction = {
        triggered: false,
        async: false,
        reason: ctx.userId ? 'no_ctx' : 'anonymous',
      }
    }
    ...
    if (
      !ctx.streamingMode &&
      ctx.groundedness !== null &&
      ctx.groundedness !== undefined &&
      ctx.groundedness < pipelineConfig.groundedness_flag_threshold
    ) {
      ...
      if (pipelineConfig.semantic_cache_enabled && ctx.earlyQueryVector) {
        ctx.waitUntilCtx.waitUntil(
          queryService.storeSemanticCache(
            `sc:${queryService.hashQuery(ctx.request.query)}`,
            ctx.earlyQueryVector,
            ctx.cacheKey
          )
        )
      }
