...
export const llmGenerationStep: PipelineStep = {
  ...
  async execute(ctx: PipelineContext): Promise<PipelineContext> {
    ...
    if (ctx.queryType === 'general-knowledge') {
      ...
      if (llmResult.usage) {
        tokenBreakdown.main_generation = {
          ...llmResult.usage,
          model: effectiveLlmModel,
          estimated: false,
        }
    ...
    if (ctx.streamingMode && ctx.onToken) {
      rawLLMAnswer = await queryService.streamLLMGeneration(
        effectiveLlmModel,
        llmMessages,
        pipelineConfig.max_tokens_generation,
        gatewayOptions,
        ctx.onToken
      )
      ...
      const llmResult = (await (env.AI.run as Function)(
        ...
        ragParams,
        gatewayOptions
      )) as LLMResponse & { thinking?: string }
