...
export const selfReflectionStep: PipelineStep = {
  ...
  async execute(ctx: PipelineContext): Promise<PipelineContext> {
    const { env, pipelineConfig, prompts, gatewayOptions, tokenBreakdown, trace, queryService } =
      ctx
    ...
    // Judge 驅動重生成
    const regenQualityTriggered =
      quality != null && quality <= pipelineConfig.judge_regen_quality_max
    const shouldRegen =
      regenQualityTriggered &&
      ctx.queryType === 'complex' &&
      !ctx.cannotAnswer &&
      parsedAnswer.length >= pipelineConfig.self_reflection_min_length

    if (!shouldRegen) return ctx

    const regenGroundednessTriggered =
      groundedness != null && groundedness < (pipelineConfig.groundedness_disclaimer_low ?? 0.6)
    ...
    try {
      ...
      if (retryResult.usage) {
        tokenBreakdown.self_reflection_regen = {
          ...retryResult.usage,
          model: effectiveLlmModel,
          estimated: false,
        }
      ...
      const regenAnswer =
        !ctx.cannotAnswer && finalSources.length > 0
