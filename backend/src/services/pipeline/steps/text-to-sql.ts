  ...
  skipWhen: [
    {
      field: 'queryType',
      operator: 'in',
      value: ['simple', 'complex', 'general-knowledge', 'multi-tool'],
    },
  ],
...
async function handleSqlPath(
  ...
  try {
    ...
    // 追蹤 token 使用
    if (llmResult.usage) {
      ctx.tokenBreakdown.text_to_sql = {
        ...llmResult.usage,
        model: pipelineConfig.lightweight_model,
        estimated: false,
      }
    ...
    const queryId = await queryService.logQuery({
      ...
      tokenCount: totalTokens > 0 ? totalTokens : (llmResult.usage?.total_tokens ?? null),
      queryType: 'sql',
...
async function handleHybridPath(
  ...
  try {
    ...
    const excludedIds =
      ctx.climbed_route_ids && ctx.climbed_route_ids.length > 0 ? ctx.climbed_route_ids : undefined
...
async function emptyPersonalResponse(
  ctx: PipelineContext,
  template: string
): Promise<PipelineContext> {
  const { request, trace, queryService } = ctx
