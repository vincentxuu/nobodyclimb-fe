...
export class BiographyService {
   ...
   */
  async getList(options: BiographyQueryOptions): Promise<PaginatedResponse<BiographyListResponse>> {
  ...
  async createOrUpdate(
    ...
    if (existing) {
      ...
      if (data.one_liners_data !== undefined) {
        const oneLinersData =
          typeof data.one_liners_data === 'string'
      ...
      if (data.stories_data !== undefined) {
        const storiesData =
          typeof data.stories_data === 'string' ? JSON.parse(data.stories_data) : data.stories_data
