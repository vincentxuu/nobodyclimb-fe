...
import { z } from 'zod'
import { PostListItem, PostQueryOptions, PostRepository } from '../repositories/post-repository'
...
export class PostService {
  ...
  async create(
    ...
    // 創建 post
    const _post = await this.repository.create({
      id,
      author_id: userId,
      title: data.title,
      slug,
   ...
   * 刪除 post
   */
  async delete(id: string, userId: string, userRole: string, storage: R2Bucket): Promise<void> {
    // 檢查權限
    const existing = await this.repository.findById(id)
   ...
   */
  private async cachePostMetadata(post: PostListItem, tags: string[]): Promise<void> {
    const rawMetadata = {
      id: post.id,
      title: post.title,
