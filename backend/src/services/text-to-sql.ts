...
export class TextToSqlService {
  ...
  async validateRouteName(
    ...
    try {
      ...
      const exactResult = await this.db
        .prepare(exactSql)
        .bind(...exactParams)
        .all<{ id: string; name: string; crag_id: string }>()
  ...
  async queryCandidates(
    ...
    try {
      ...
      let sql =
        'SELECT r.id, r.name, r.grade, r.route_type, r.description, r.bolt_count, r.height, c.name as crag_name FROM routes r JOIN crags c ON r.crag_id = c.id WHERE 1=1'
      const binds: unknown[] = []
  ...
  private async countRoutesAtCrag(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    const result = await this.db
      .prepare('SELECT COUNT(*) as count FROM routes WHERE crag_id = ?')
      .bind(cragId)
      .all<Record<string, unknown>>()
    return result.results || []
  }

  private async listRoutesByCriteria(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    const gradeMin = params.grade_min as string | undefined
    ...
    let sql =
      'SELECT r.name, r.grade, r.route_type, r.description, r.bolt_count, r.height FROM routes r WHERE r.crag_id = ?'
  ...
  private async listRoutesAtGrade(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    ...
    const result = await this.db
      ...
      .bind(cragId, gradeParam, limit)
      .all<Record<string, unknown>>()
    return result.results || []
  }

  private async routeInfoLookup(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
  ...
  private async cragInfoLookup(
    ...
    const [cragResult, areaResult] = await Promise.all([
      this.db
        ...
        .all<Record<string, unknown>>(),
      this.db
        .prepare('SELECT COUNT(*) as area_count FROM areas WHERE crag_id = ?')
        .bind(cragId)
        .all<Record<string, unknown>>(),
    ])
  ...
  private async rankCragsByRoutes(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
  ...
  private async gradeDistribution(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    const [totalResult, gradeResult] = await Promise.all([
      this.db
        .prepare('SELECT COUNT(*) as total FROM routes WHERE crag_id = ?')
        .bind(cragId)
        .all<{ total: number }>(),
      this.db
        .prepare(
          'SELECT grade, COUNT(*) as count FROM routes WHERE crag_id = ? AND grade IS NOT NULL GROUP BY grade ORDER BY grade ASC'
        )
        .bind(cragId)
        .all<Record<string, unknown>>(),
    ])
  ...
  private async routeTypeDistribution(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    const result = await this.db
  ...
  private async routeFirstAscent(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const routeId = this.requireParam(params, 'route_id')
    const result = await this.db
      .prepare('SELECT name, grade, first_ascent FROM routes WHERE id = ?')
      .bind(routeId)
      .all<Record<string, unknown>>()
    return result.results || []
  }
  ...
  private async listVideosForRoute(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const routeId = this.requireParam(params, 'route_id')
    const result = await this.db
  ...
  private async routesWithVideos(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const cragId = this.requireParam(params, 'crag_id')
    const limit = this.safeLimit(params, 'limit', 50, 50)
  ...
  private async myAscentCount(params: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    ...
    const result = await this.db
      .prepare('SELECT COUNT(*) as count FROM user_route_ascents WHERE user_id = ?')
      .bind(userId)
      .all<Record<string, unknown>>()
    return result.results || []
  }

  private async myAscentByType(
    params: Record<string, unknown>
  ): Promise<Record<string, unknown>[]> {
    const userId = this.ensureUserId(params)
  ...
  private async myAscentByDate(
    ...
    let sql =
      'SELECT r.name, r.grade, r.route_type, ura.ascent_type, ura.ascent_date, c.name as crag_name FROM user_route_ascents ura JOIN routes r ON ura.route_id = r.id JOIN crags c ON r.crag_id = c.id WHERE ura.user_id = ?'
  ...
  private async myRatedRoutes(params: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    ...
    let sql =
      'SELECT r.name, r.grade, r.route_type, ura.rating, ura.ascent_date, c.name as crag_name FROM user_route_ascents ura JOIN routes r ON ura.route_id = r.id JOIN crags c ON r.crag_id = c.id WHERE ura.user_id = ? AND ura.rating IS NOT NULL'
    const binds: unknown[] = [userId]
