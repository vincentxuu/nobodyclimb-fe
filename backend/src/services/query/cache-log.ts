import type { Env, AIAskResponse, AISource } from '../../types';
...
// 記錄查詢日誌，回傳 query_id
export async function logQuery(db: D1Database, params: {
  userId: string | null;
  query: string;
  response: string;
  sources: AISource[];
...
// 將低品質回應寫入審核佇列
export async function flagResponse(db: D1Database, queryLogId: string, reason: 'low_groundedness' | 'low_feedback' | 'score_discrepancy'): Promise<void> {
  try {
    await db.prepare(`
