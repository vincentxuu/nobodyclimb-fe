...
export function extractGradeFilter(query: string): { $gte: number; $lte: number } | null {
  ...
  const maxMatch = allMatches.reduce((prev, curr) => {
    const prevNum =
      parseInt(prev[1], 10) * 10 + (prev[2] ? 'abcd'.indexOf(prev[2].toLowerCase()) : 0)
    const currNum =
      parseInt(curr[1], 10) * 10 + (curr[2] ? 'abcd'.indexOf(curr[2].toLowerCase()) : 0)
    return currNum > prevNum ? curr : prev
  })
  // 若最大值的 grade 沒有 a-d 後綴，擴展到 +3（含 a/b/c/d 子等級）
  const maxBase =
    parseInt(maxMatch[1], 10) * 10 + (maxMatch[2] ? 'abcd'.indexOf(maxMatch[2].toLowerCase()) : 0)
