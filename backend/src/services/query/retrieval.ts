import type { AIDocument, AIDocumentMetadata, Env } from '../../types'
...
export async function searchBM25(
  ...
  try {
    ...
    `)
      .bind(ftsQuery, topK)
      .all<{ doc_id: string; bm25_score: number }>()
  ...
  while (selected.length < k && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      for (const sel of selected) {
        const sim = documentSimilarity(documents.get(candidate.id), documents.get(sel.id))
...
async function runAgenticSearch(
  ...
  const vecPromise = !skipVector
    ...
    : Promise.resolve({
        matches: [] as Array<{ id: string; score: number; metadata?: Record<string, unknown> }>,
      })
  ...
  const vecMatches: SearchResult[] = vecResult.matches.map((m) => ({
    id: m.id,
    score: m.score,
    metadata: m.metadata,
  }))
...
async function decideNextAction(
  ...
  try {
    ...
    if (
      !['ANSWER', 'RETRIEVE', 'BROADEN', 'SWITCH_TOOL', 'DECOMPOSE', 'VERIFY'].includes(parsed.type)
    )
...
export async function agenticRetrieve(
  ...
  candidates: SearchResult[]
  terminationReason: 'enough_docs' | 'max_steps' | 'no_improvement'
  initialSearch: {
  ...
  // Step 0：初始搜尋
  const initialResults = await runAgenticSearch(
    deps,
    query,
    vectorFilter,
    MERGE_TOP_K,
  ...
  const initialSearchTrace = {
    ...
    quality_check: undefined as
      | { unique_count: number; avg_score: number; passed: boolean }
      | undefined,
  }
  ...
  for (let step = 0; step < cfg.agentic_max_steps; step++) {
    ...
    if (uniqueCount >= cfg.agentic_min_docs_to_answer) {
      ...
      if (step === 0 || !initialSearchTrace.quality_check) {
        initialSearchTrace.quality_check = {
          unique_count: uniqueCount,
          avg_score: avgScore,
          passed: avgScore >= cfg.agentic_min_quality_score,
        }
      }
    ...
    const { action, usage: decisionUsage } = await decideNextAction(
      ...
      merged,
      step,
      cfg.agentic_max_steps,
      cfg.agentic_min_docs_to_answer,
    ...
    if (action.type === 'ANSWER') {
      steps.push({ step, type: action.type, docs_retrieved: merged.length } as AgenticStepTrace & {
    ...
    if (action.type === 'RETRIEVE') {
      if (!action.refinedQuery) {
        steps.push({ step, type: 'ANSWER', docs_retrieved: merged.length } as AgenticStepTrace & {
          docs_retrieved: number
        })
        agenticTerminationReason = 'no_improvement'
        break
      ...
      const newResults = await runAgenticSearch(
        deps,
        action.refinedQuery,
        vectorFilter,
        MERGE_TOP_K,
      ...
      steps.push({
        step,
        type: action.type,
        refinedQuery: action.refinedQuery,
        docs_retrieved: newResults.length,
      ...
      const broadenResults = await runAgenticSearch(
        deps,
        query,
        broadenFilter,
        MERGE_TOP_K,
      ...
      steps.push({
        step,
        type: action.type,
        docs_retrieved: broadenResults.length,
      } as AgenticStepTrace & { docs_retrieved: number })
      ...
      const switchResults = await runAgenticSearch(
        deps,
        query,
        switchFilter,
        MERGE_TOP_K,
      ...
      const subResults = await Promise.all(
        action.subQueries
          .slice(0, 3)
          .map((sq) => runAgenticSearch(deps, sq, vectorFilter, MERGE_TOP_K, cfg.bm25_top_k))
      )
      const totalDocs = subResults.reduce((sum, r) => sum + r.length, 0)
      steps.push({
        step,
        type: action.type,
        subQueries: action.subQueries,
      ...
      verifyUsed = true
      const verifyResults = await runAgenticSearch(
        deps,
        action.verifyQuery,
        {},
        MERGE_TOP_K,
      ...
      steps.push({
        step,
        type: action.type,
        verifyQuery: action.verifyQuery,
  ...
  return {
    ...
    terminationReason: agenticTerminationReason,
    initialSearch: initialSearchTrace,
  }
}
