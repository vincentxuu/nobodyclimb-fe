...
export class QueryService {
  ...
  private get langfuseParent(): LangfuseParent | null {
    return (
      this._pipelineCtx?.currentLfSpan ??
      this._pipelineCtx?.langfuseTrace ??
      null
    );
  }
  ...
  async ask(
    ...
    // 對話歷史
    const recentHistory: AIChatMessage[] = chat_history
      ? chat_history.slice(-6)
      : [];
    const historyHash =
    ...
    // 快取鍵
    const personalizedContext = [memorySummary, ascentContext]
      .filter(Boolean)
      .join("|");
    const personalizedHash = personalizedContext
      ? `:p${this.hashQuery(personalizedContext)}`
      : "";
    ...
    const isAnonymousNoHistory =
      !userId && recentHistory.length === 0 && !no_cache;
    ...
    const p = {
      ...
      HYDE_PROMPT: resolvePrompt(dbPrompts["hyde_prompt"], HYDE_PROMPT, [
        "query",
      ]),
      ...
      PLANNING_PROMPT: resolvePrompt(
        dbPrompts["planning_prompt"],
        PLANNING_PROMPT,
        ["query", "crags", "areas", "max_steps"],
      ),
      SYNTHESIS_PROMPT: resolvePrompt(
        dbPrompts["synthesis_prompt"],
        SYNTHESIS_PROMPT,
        ["query", "step_results"],
      ),
      QUERY_TEMPLATE: resolvePrompt(
        dbPrompts["query_template"],
        QUERY_TEMPLATE,
        ["context", "query"],
      ),
    };
  ...
  async search(
  ...
  ): Promise<{ results: AISource[]; count: number }> {
    ...
    const relevantMatches = searchResults.matches.filter(
      (m) => m.score >= cfg.min_vector_score,
    );
