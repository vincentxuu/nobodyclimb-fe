...
async function executeStepInner(
  ...
  if (step.tool === 'sql_query') {
    try {
      ...
      if (cragName) {
        const row = await deps.env.DB.prepare(`SELECT id FROM crags WHERE name LIKE ? LIMIT 1`)
          .bind(`%${cragName}%`)
          .first<{ id: string }>()
        cragId = row?.id
      }
  ...
  const candidates = await runPlanSearch(
    deps,
    step.query,
    vectorFilter,
    cfg.merge_top_k,
...
export async function executePlan(
  ...
  let adaptiveReplanInfo:
    | { trigger_step_id: number; reason: string; new_steps: PlanStep[] }
    | undefined
  ...
  while (remaining.length > 0) {
    ...
    const execResults = await Promise.all(executable.map((step) => executeStep(deps, step, cfg)))
...
function fallbackSynthesis(stepResults: StepExecutionResult[]): {
  context: string
  sources: AISource[]
} {
  const parts: string[] = []
...
export async function synthesize(
  ...
  if (
    ...
    stepResults.every((r) => r.candidates.length === 0 && !r.sqlContext)
  ) {
  ...
  const prompt = template.replace('{query}', query).replace('{step_results}', stepResultsText)
