...
export async function parseQueryWithLLM(
  ...
  try {
    ...
    if (
      !parsed.query_type ||
      !['simple', 'complex', 'general-knowledge', 'sql', 'hybrid', 'clarification-needed'].includes(
        parsed.query_type
      )
    ) {
    ...
    if (parsed.tool === 'multi_tool') {
      const validSubTools = toolRegistry
        .getValidToolNames()
        .filter((t) => t !== 'multi_tool' && t !== 'general_knowledge')
      if (
        !parsed.multi_tool?.steps ||
        !Array.isArray(parsed.multi_tool.steps) ||
        parsed.multi_tool.steps.length === 0
      ) {
        parsed.tool = 'search_routes'
        parsed.multi_tool = undefined
      } else {
        const validSteps = parsed.multi_tool.steps.filter(
          (s) => s.tool && validSubTools.includes(s.tool)
        )
...
export async function runJudge(
  ...
  try {
    ...
    return {
      ...scores,
      rawResponse,
      contextChars: truncatedContext.length,
      contextTruncated,
      usage,
    }
  } catch (err) {
