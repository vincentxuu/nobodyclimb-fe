...
ascentsRoutes.get(
  ...
  describeRoute({
    ...
    description:
      '取得已登入使用者的所有攀爬記錄，支援路線、岩場、攀爬類型、日期範圍等篩選條件，並支援分頁',
    responses: {
...
ascentsRoutes.get(
  ...
  describeRoute({
    ...
    summary: '取得攀爬統計',
    description:
      '取得當前使用者的攀爬統計資料，包含總攀爬次數、唯一路線數、唯一岩場數、按攀爬類型統計、按難度統計、最近 12 個月統計、最高難度及最近攀爬記錄',
    responses: {
  ...
  async (c) => {
    ...
    if (year) {
      ...
      if (isNaN(yearNum) || yearNum < 1900 || yearNum > new Date().getFullYear() + 10) {
        return c.json(
          { success: false, error: 'Bad Request', message: 'Invalid year parameter' },
          400
        )
      }
    ...
    return c.json({
      ...
      data: {
        ...
        by_grade: byGrade.results.reduce(
          (acc, item) => {
        ...
        highest_grades: highestGrades.results.reduce(
          (acc, item) => {
...
ascentsRoutes.get(
  ...
  describeRoute({
    ...
    summary: '取得單筆攀爬記錄',
    description:
      '依據 ID 取得單筆攀爬記錄的詳細資訊，包含路線、岩場及使用者資料。私人記錄僅擁有者可查看',
    responses: {
...
ascentsRoutes.post(
  ...
  describeRoute({
    ...
    summary: '新增攀爬記錄',
    description:
      '為當前使用者新增一筆攀爬記錄，需提供路線 ID、攀爬類型及攀爬日期。可選填嘗試次數、評分、感知難度、筆記、照片、影片連結等',
    responses: {
  ...
  async (c) => {
    ...
    if (!body.ascent_type) {
      return c.json(
        { success: false, error: 'Bad Request', message: 'ascent_type is required' },
        400
      )
    }
    ...
    if (!body.ascent_date) {
      return c.json(
        { success: false, error: 'Bad Request', message: 'ascent_date is required' },
        400
      )
    }
    ...
    if (isPublic) {
      await c.env.DB.prepare('UPDATE routes SET ascent_count = ascent_count + 1 WHERE id = ?')
        .bind(body.route_id)
        .run()
    }
...
ascentsRoutes.put(
  ...
  describeRoute({
    ...
    summary: '更新攀爬記錄',
    description:
      '更新指定 ID 的攀爬記錄，僅記錄擁有者可進行更新。可更新攀爬類型、日期、嘗試次數、評分、感知難度、筆記、照片、影片連結及公開狀態',
    responses: {
  ...
  async (c) => {
    ...
    if (existing.user_id !== userId) {
      return c.json(
        { success: false, error: 'Forbidden', message: 'You can only edit your own ascents' },
        403
      )
    }
    ...
    await c.env.DB.prepare(`UPDATE user_route_ascents SET ${updates.join(', ')} WHERE id = ?`)
      .bind(...values)
      .run()
...
ascentsRoutes.delete(
  ...
  async (c) => {
    ...
    if (existing.user_id !== userId) {
      return c.json(
        { success: false, error: 'Forbidden', message: 'You can only delete your own ascents' },
        403
      )
    }
...
ascentsRoutes.get(
  ...
  describeRoute({
    ...
    summary: '取得路線攀爬摘要',
    description:
      '取得指定路線的攀爬統計摘要，包含總攀爬次數、唯一攀爬者數、平均評分及按攀爬類型統計',
    responses: {
