...
import { generateId, parsePagination, safeJsonParse, toBool } from '../utils/id'
...
const createRouteStorySchema = z.object({
  ...
  story_type: z
    .enum(['beta', 'experience', 'history', 'safety', 'other'])
    .optional()
    .default('other'),
...
routeStoriesRoutes.get(
  ...
  describeRoute({
    ...
    summary: '取得路線故事列表',
    description:
      '取得所有公開的路線故事列表，支援分頁和篩選（route_id、crag_id、story_type、featured）',
    responses: {
...
routeStoriesRoutes.get(
  ...
  async (c) => {
    ...
    if (story.visibility === 'community' && !userId) {
      return c.json(
        { success: false, error: 'Unauthorized', message: 'Login required to view this story' },
        401
      )
    }
...
routeStoriesRoutes.post(
  ...
  async (c) => {
    ...
    if (visibility === 'public') {
      await c.env.DB.prepare('UPDATE routes SET story_count = story_count + 1 WHERE id = ?')
        .bind(body.route_id)
        .run()
    }
...
routeStoriesRoutes.put(
  ...
  async (c) => {
    ...
    if (existing.user_id !== userId) {
      return c.json(
        { success: false, error: 'Forbidden', message: 'You can only edit your own stories' },
        403
      )
    }
    ...
    await c.env.DB.prepare(`UPDATE route_stories SET ${updates.join(', ')} WHERE id = ?`)
      .bind(...values)
      .run()
...
routeStoriesRoutes.delete(
  ...
  async (c) => {
    ...
    if (existing.user_id !== userId) {
      return c.json(
        { success: false, error: 'Forbidden', message: 'You can only delete your own stories' },
        403
      )
    }

    // 刪除相關互動
    await c.env.DB.prepare("DELETE FROM likes WHERE entity_type = 'route_story' AND entity_id = ?")
      .bind(id)
      .run()
    await c.env.DB.prepare(
      "DELETE FROM comments WHERE entity_type = 'route_story' AND entity_id = ?"
    )
      .bind(id)
      .run()
    await c.env.DB.prepare(
      "DELETE FROM content_reactions WHERE content_type = 'route_story' AND content_id = ?"
    )
      .bind(id)
      .run()
    ...
    if (existing.visibility === 'public') {
      await c.env.DB.prepare('UPDATE routes SET story_count = MAX(0, story_count - 1) WHERE id = ?')
...
routeStoriesRoutes.post(
  ...
  async (c) => {
    ...
    if (existing) {
      ...
      await c.env.DB.prepare('UPDATE route_stories SET like_count = like_count - 1 WHERE id = ?')
        .bind(storyId)
        .run()
...
routeStoriesRoutes.get(
  ...
  async (c) => {
    ...
    let whereClause =
      "s.route_id = ? AND (s.visibility = 'public' OR (s.visibility = 'community' AND ? IS NOT NULL))"
