...
gymsRoutes.get(
  ...
  async (c) => {
    ...
    const gym = await c.env.DB.prepare('SELECT * FROM gyms WHERE id = ?').bind(id).first<Gym>()
...
gymsRoutes.get(
  ...
  async (c) => {
    ...
    const gym = await c.env.DB.prepare('SELECT * FROM gyms WHERE slug = ?').bind(slug).first<Gym>()
...
gymsRoutes.put(
  ...
  async (c) => {
    ...
    const existing = await c.env.DB.prepare('SELECT id FROM gyms WHERE id = ?').bind(id).first()
