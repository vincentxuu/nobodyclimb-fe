...
const getPhotoById = async (db: D1Database, photoId: string) => {
  return db.prepare(`${PHOTO_DETAIL_SQL} WHERE gi.id = ?`).bind(photoId).first()
}
...
galleriesRoutes.get(
  ...
  async (c) => {
    ...
    const [countResult, photosResult] = await c.env.DB.batch([
      ...
      c.env.DB.prepare(`${PHOTO_DETAIL_SQL} ORDER BY gi.created_at DESC LIMIT ? OFFSET ?`).bind(
        limit,
        offset
      ),
    ])
...
galleriesRoutes.put(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE gallery_images SET ${updates.join(', ')} WHERE id = ?`)
      .bind(...values)
      .run()
...
galleriesRoutes.delete(
  ...
  describeRoute({
    ...
    summary: '刪除照片',
    description:
      '刪除指定的照片，同時從 R2 儲存空間移除圖片檔案。只能刪除自己上傳的照片，管理員可以刪除任何照片。',
    responses: {
  ...
  async (c) => {
    ...
    await c.env.DB.prepare('DELETE FROM gallery_images WHERE id = ?').bind(photoId).run()
...
galleriesRoutes.get(
  ...
  describeRoute({
    ...
    summary: '取得單一相簿詳情',
    description:
      '依 ID 取得相簿詳細資訊，包含所有照片。會追蹤瀏覽次數（同一 IP 24 小時內只計算一次）。',
    responses: {
...
galleriesRoutes.get(
  ...
  describeRoute({
    ...
    description:
      '依 Slug 取得相簿詳細資訊，包含所有照片。會追蹤瀏覽次數（同一 IP 24 小時內只計算一次）。',
    responses: {
      200: { description: '成功取得相簿詳情' },
      404: { description: '相簿不存在' },
    },
      ...
      for (let i = 0; i < body.images.length; i++) {
        ...
        await c.env.DB.prepare(
         ...
         VALUES (?, ?, ?, ?, ?, ?)`
          ...
          .bind(generateId(), id, img.image_url, img.thumbnail_url || null, img.caption || null, i)
          .run()
      }
    }

    const gallery = await c.env.DB.prepare('SELECT * FROM galleries WHERE id = ?').bind(id).first()
...
galleriesRoutes.put(
  ...
  describeRoute({
    ...
    summary: '更新相簿資訊',
    description:
      '更新相簿的標題、描述、封面圖片或精選狀態。只能更新自己建立的相簿，管理員可以更新任何相簿。',
    responses: {
  ...
  async (c) => {
    ...
    const existing = await c.env.DB.prepare('SELECT author_id FROM galleries WHERE id = ?')
      .bind(id)
    ...
    await c.env.DB.prepare(`UPDATE galleries SET ${updates.join(', ')} WHERE id = ?`)
      .bind(...values)
      .run()
...
galleriesRoutes.delete(
  ...
  describeRoute({
    ...
    summary: '刪除相簿',
    description:
      '刪除指定的相簿及其所有照片。會同時從 R2 儲存空間移除所有圖片檔案。只能刪除自己建立的相簿，管理員可以刪除任何相簿。',
    responses: {
