...
import { Biography, Env } from '../types'
...
/** 所有故事欄位 */
const _ALL_STORY_FIELDS = [...CORE_STORY_FIELDS, ...ADVANCED_STORY_FIELDS] as const
 ...
 */
async function invalidateBiographyCaches(cache: KVNamespace, slug: string | null): Promise<void> {
...
biographiesRoutes.put(
  ...
  async (c) => {
    ...
    const autosaveFields = ['tags_data', 'basic_info_data']
    ...
    if (updates.length > 0) {
      ...
      await c.env.DB.prepare(`UPDATE biographies SET ${updates.join(', ')} WHERE id = ?`)
        .bind(...values)
        .run()
    }
    ...
    if (body.one_liners_data !== undefined) {
      const oneLinersData =
        typeof body.one_liners_data === 'string'
    ...
    if (body.stories_data !== undefined) {
      const storiesData =
        typeof body.stories_data === 'string'
...
biographiesRoutes.delete(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare('DELETE FROM biographies WHERE id = ?').bind(existing.id).run()
...
biographiesRoutes.post(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO follows (id, follower_id, following_id) VALUES (?, ?, ?)')
    ...
    const follower = await c.env.DB.prepare('SELECT display_name, username FROM users WHERE id = ?')
      .bind(userId)
...
biographiesRoutes.get(
  ...
  `
    ...
    const countResult = await (country ? countStmt.bind(country) : countStmt).first<{
      total: number
    }>()
  ...
  `
    ...
    if (locations.length > 0) {
      const locationConditions = locations
        .map(() => '(cl.location = ? AND cl.country = ?)')
        .join(' OR ')
...
biographiesRoutes.get(
  ...
  async (c) => {
    ...
    const unlockedBadges = await c.env.DB.prepare('SELECT * FROM user_badges WHERE user_id = ?')
      .bind(biography.user_id)
      .all()
    ...
    const unlockedBadgesMap = new Map(
      (unlockedBadges.results || []).map((b: { badge_id?: string; unlocked_at?: string }) => [
        b.badge_id,
        b.unlocked_at,
      ])
    )
    ...
    return c.json({
      success: true,
      data: {
        unlocked: (unlockedBadges.results || []) as Array<{
          id: string
          user_id: string
          badge_id: string
          unlocked_at: string
...
biographiesRoutes.get(
  ...
  async (c) => {
    ...
    const statsData = {
      ...
      trending_categories: (trendingCategories.results || []).map(
...
biographiesRoutes.get(
  ...
  async (c) => {
    ...
    const leaderboard = (results.results || []).map(
      (
        item: {
          biography_id?: string
          name?: string
...
biographiesRoutes.post(
  ...
  async (c) => {
    ...
    if (existingLike) {
      ...
      await c.env.DB.prepare('DELETE FROM biography_likes WHERE user_id = ? AND biography_id = ?')
        .bind(userId, biographyId)
        .run()
      liked = false
    } else {
    ...
    await c.env.DB.prepare('UPDATE biographies SET total_likes = ? WHERE id = ?')
