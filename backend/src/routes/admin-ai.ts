...
import { STEP_REGISTRY } from '../services/pipeline/registry'
import type { BranchConfig, PipelineStepConfig } from '../services/pipeline/types'
import { getUserRankDetail, recalculateAllRanks, updateUserRank } from '../services/rank'
...
adminAiRoutes.get(
  ...
  async (c) => {
    try {
      // 總查詢數
      const totalRow = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM ai_query_logs`).first<{
        count: number
      }>()
      ...
      const successRate =
        feedbackRow && feedbackRow.total > 0 ? feedbackRow.positive / feedbackRow.total : null
    ...
    try {
      const [summary, byType] = await Promise.all([
        ...
        c.env.DB.prepare(
          ...
          .bind(...bindings)
          .all<{ query_type: string | null; count: number }>(),
      ])
...
adminAiRoutes.get(
  ...
  async (c) => {
    ...
    if (feedback_min) {
      conditions.push('l.feedback_score >= ?')
      bindings.push(parseInt(feedback_min, 10))
    }
    if (feedback_max) {
      conditions.push('l.feedback_score <= ?')
      bindings.push(parseInt(feedback_max, 10))
    }
    if (query_type) {
      conditions.push('l.query_type = ?')
      bindings.push(query_type)
    }
    if (search) {
      conditions.push('l.query LIKE ?')
...
adminAiRoutes.get(
  ...
  describeRoute({
    ...
    responses: {
      200: { description: '日誌詳細資料與各階段流程' },
      404: { description: '找不到日誌' },
    },
  }),
  async (c) => {
    ...
    try {
      const [log, flagRows] = await Promise.all([
        ...
        c.env.DB.prepare(
          ...
          .bind(id)
          .all<{ flag_reason: string; is_reviewed: number; created_at: string }>(),
      ])
      ...
      let sources: unknown[] = []
      try {
        sources = JSON.parse((log.sources as string) ?? '[]')
      } catch {
        /* ignore */
      }
      ...
      const pipeline = {
        ...
        query_parsing: {
          ...
          description:
            'LLM A：Tool Calling 解析查詢意圖（search_routes / search_crags / general_knowledge）',
        ...
        text_to_sql: {
          ...
          candidate_count:
            typeof (pt?.text_to_sql as Record<string, unknown> | undefined)?.candidate_count ===
            'number'
              ? (pt?.text_to_sql as Record<string, unknown> | undefined)?.candidate_count
              : null,
          skipped:
            isCacheHit ||
            !['sql', 'hybrid', 'clarification-needed'].includes((log.query_type as string) ?? ''),
...
adminAiRoutes.get(
  ...
  async (c) => {
    try {
      const routeTotal = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM routes`).first<{
        count: number
      }>()
      ...
      const cragTotal = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM crags`).first<{
        count: number
      }>()
...
const PROMPT_DEFAULTS = [
  ...
  {
    name: 'general_knowledge_system_prompt',
    label: '通識知識提示詞',
  ...
  {
    name: 'contextual_chunk_prompt',
    label: 'Contextual RAG 語意摘要',
    content: CONTEXTUAL_CHUNK_PROMPT,
    variables: ['type', 'content'],
  },
  {
    name: 'multi_query_expansion_prompt',
    label: 'Multi-Query 查詢擴展',
    content: MULTI_QUERY_EXPANSION_PROMPT,
    variables: ['query', 'count'],
  },
  {
    name: 'agentic_decision_prompt',
    label: 'Agentic 決策',
    content: AGENTIC_DECISION_PROMPT,
  ...
  async (c) => {
    ...
    try {
      ...
      await c.env.DB.prepare(
        ...
        .bind(id, name, version, content, JSON.stringify(variables), effectiveStatus)
        .run()
...
adminAiRoutes.get(
  ...
  async (c) => {
    const id = c.req.param('id')
    try {
      const prompt = await c.env.DB.prepare(`SELECT * FROM ai_prompts WHERE id = ?`)
        .bind(id)
        .first()
      if (!prompt)
  ...
  async (c) => {
    ...
    try {
      ...
      if (updates.variables !== undefined) {
        ...
        vals.push(JSON.stringify(updates.variables))
      }
      ...
      if (fields.length === 0) {
        return c.json(
          { success: false, error: 'ValidationError', message: '沒有提供要更新的欄位' },
          400
        )
      }
...
adminAiRoutes.delete(
  ...
  async (c) => {
    const id = c.req.param('id')
    try {
      const result = await c.env.DB.prepare(`DELETE FROM ai_prompts WHERE id = ?`).bind(id).run()
      if (result.meta.changes === 0) {
        return c.json({ success: false, error: 'NotFound', message: '找不到 Prompt' }, 404)
      }
...
adminAiRoutes.get(
  ...
  async (c) => {
    try {
      const rows = await c.env.DB.prepare(`SELECT key, value FROM ai_config ORDER BY key`).all<{
        key: string
        value: string
      }>()
...
adminAiRoutes.get(
  ...
  async (c) => {
    ...
    try {
      ...
      if (!detail)
        return c.json({ success: false, error: 'NotFound', message: '該用戶尚無等級記錄' }, 404)
...
adminAiRoutes.put(
  ...
  async (c) => {
    ...
    try {
      if (rank === null) {
        await c.env.DB.prepare(
          `UPDATE user_ranks SET rank_override_id = NULL, updated_at = datetime('now') WHERE user_id = ?`
        )
          .bind(userId)
          .run()
        ...
        await c.env.DB.prepare(
          `UPDATE user_ranks SET rank_override_id = ?, rank_id = ?, daily_ai_limit = ?, updated_at = datetime('now') WHERE user_id = ?`
        )
      ...
      return c.json({
        success: true,
        message: rank ? `已覆寫等級為「${rank}」` : '已清除覆寫，恢復自動計算',
      })
    } catch (error) {
    ...
    try {
      ...
      const getPercentile = async (col: string, pct: number): Promise<number | null> => {
        ...
        const row = await c.env.DB.prepare(
          ...
          .bind(offset)
          .first<{ val: number | null }>()
        return row?.val ?? null
      }

      const [embeddingP50, embeddingP95, retrievalP50, retrievalP95, generationP50, generationP95] =
        await Promise.all([
...
adminAiRoutes.get(
  ...
  async (c) => {
    try {
      ...
      return c.json(
        { success: false, error: 'DatabaseError', message: '讀取 pipeline 設定失敗' },
        500
      )
    }
  }
...
adminAiRoutes.put(
  ...
  async (c) => {
    try {
      const { steps } = c.req.valid('json' as never) as {
        steps: Array<{ id: string; enabled: boolean; order: number }>
      }
      ...
      // 審計日誌
      const _adminId = c.get('userId') as string | undefined
      const _changed = steps.map((s) => `${s.id}:${s.enabled ? 'on' : 'off'}:${s.order}`).join(', ')

      // 寫入 ai_config
      await c.env.DB.prepare(
        `INSERT OR REPLACE INTO ai_config (key, value) VALUES ('pipeline_steps', ?)`
      )
        .bind(JSON.stringify(steps))
        .run()
      ...
      return c.json(
        { success: false, error: 'DatabaseError', message: '更新 pipeline 設定失敗' },
        500
      )
    }
  }
...
adminAiRoutes.get(
  ...
  async (c) => {
    try {
      ...
      `)
        .bind(days)
        .all<{ date: string; kv_hits: number; semantic_hits: number }>()
      ...
      const daily = dailyBasicRows.results.map((row) => {
        ...
        const qt = queryTypeMap.get(row.date) ?? {
          simple: 0,
          complex: 0,
          'general-knowledge': 0,
          guardrails_blocked: 0,
        ...
        return {
          ...
          quality: {
            avg_groundedness:
              row.avg_groundedness != null ? Math.round(row.avg_groundedness * 100) / 100 : null,
            avg_auto_score:
              row.avg_auto_score != null ? Math.round(row.avg_auto_score * 10) / 10 : null,
            avg_feedback_score:
              row.avg_feedback_score != null ? Math.round(row.avg_feedback_score * 10) / 10 : null,
          },
          cache: {
            hit_rate: hitRate,
      ...
      return c.json({
        ...
        data: {
          ...
          summary: {
            ...
            avg_latency_ms:
              summaryRow?.avg_latency_ms != null ? Math.round(summaryRow.avg_latency_ms) : null,
            avg_groundedness:
              summaryRow?.avg_groundedness != null
                ? Math.round(summaryRow.avg_groundedness * 100) / 100
                : null,
            cache_hit_rate:
              summaryRow?.cache_hit_rate != null
                ? Math.round(summaryRow.cache_hit_rate * 100) / 100
                : null,
          },
        },
      })
