...
climbingLocationsRoutes.post(
  ...
  async (c) => {
    ...
    const location = await c.env.DB.prepare('SELECT * FROM climbing_locations WHERE id = ?')
      .bind(id)
...
climbingLocationsRoutes.put(
  ...
  async (c) => {
    ...
    if (updates.length > 0) {
      ...
      await c.env.DB.prepare(`UPDATE climbing_locations SET ${updates.join(', ')} WHERE id = ?`)
        .bind(...values)
        .run()
    }
...
climbingLocationsRoutes.delete(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare('DELETE FROM climbing_locations WHERE id = ?').bind(locationId).run()
