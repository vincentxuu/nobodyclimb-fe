...
const USER_SELECT_FIELDS =
  'id, email, username, display_name, avatar_url, bio, role, google_id, auth_provider, created_at'
...
authRoutes.get(
  ...
  async (c) => {
    ...
    const user = await c.env.DB.prepare(`SELECT ${USER_SELECT_FIELDS} FROM users WHERE id = ?`)
      .bind(userId)
      .first()
...
authRoutes.post(
  ...
  async (c) => {
    ...
    try {
      ...
      let user = await c.env.DB.prepare('SELECT * FROM users WHERE google_id = ? AND is_active = 1')
        .bind(googlePayload.sub)
        .first<User>()

      // If not found by google_id, check by email
      if (!user) {
        user = await c.env.DB.prepare('SELECT * FROM users WHERE email = ? AND is_active = 1')
