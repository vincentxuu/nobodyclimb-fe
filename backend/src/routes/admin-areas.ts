...
import { Area, Env, Sector } from '../types'
...
adminSectorsRoutes.post(
  ...
  async (c) => {
    ...
    const area = await c.env.DB.prepare('SELECT * FROM areas WHERE id = ?').bind(id).first<Area>()
...
adminSectorsRoutes.delete(
  ...
  describeRoute({
    ...
    summary: '刪除岩場區域',
    description:
      '刪除區域及其下所有岩壁分區。該區域下的路線會保留但 area_id 和 sector_id 會被清空。',
    responses: {
  ...
  async (c) => {
    ...
    await c.env.DB.batch([
      c.env.DB.prepare('UPDATE routes SET area_id = NULL, sector_id = NULL WHERE area_id = ?').bind(
        areaId
      ),
      c.env.DB.prepare('DELETE FROM sectors WHERE area_id = ?').bind(areaId),
      c.env.DB.prepare('DELETE FROM areas WHERE id = ?').bind(areaId),
    ])
...
adminSectorsRoutes.put(
  ...
  async (c) => {
    ...
    const stmts = body.order.map((item) =>
      c.env.DB.prepare(
        "UPDATE areas SET sort_order = ?, updated_at = datetime('now') WHERE id = ?"
...
adminSectorsRoutes.post(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(
      ...
      .bind(id, areaId, body.name, body.name_en || null, body.sort_order || 0)
      .run()
...
adminSectorsRoutes.put(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE sectors SET ${updates.join(', ')} WHERE id = ?`)
      .bind(...values)
      .run()
...
adminSectorsRoutes.delete(
  ...
  async (c) => {
    ...
    await c.env.DB.batch([
      c.env.DB.prepare('UPDATE routes SET sector_id = NULL WHERE sector_id = ?').bind(sectorId),
      c.env.DB.prepare('DELETE FROM sectors WHERE id = ?').bind(sectorId),
    ])
