...
searchRoutes.get(
  ...
  describeRoute({
    ...
    summary: '全域搜尋',
    description:
      '搜尋文章、岩場、岩館、相簿、影片等內容。可透過 type 參數指定搜尋類型，若未指定則搜尋所有類型。',
    responses: {
  ...
  async (c) => {
    ...
    if (!type || type === 'crags') {
      const crags = await c.env.DB.prepare(
        ...
        .bind(searchPattern, searchPattern, searchPattern, searchPattern, type ? limit : 5)
        .all()
...
searchRoutes.get(
  ...
  async (c) => {
    ...
    // Get crag names
    const crags = await c.env.DB.prepare(`SELECT name FROM crags WHERE name LIKE ? LIMIT 3`)
      .bind(searchPattern)
      .all<{ name: string }>()
    suggestions.push(...crags.results.map((cr) => cr.name))

    // Get gym names
    const gyms = await c.env.DB.prepare(`SELECT name FROM gyms WHERE name LIKE ? LIMIT 3`)
      .bind(searchPattern)
      .all<{ name: string }>()
