...
usersRoutes.get(
  ...
  async (c) => {
    ...
    if (activity === 'recent_7d') {
      ...
      whereClause +=
        " AND (u.last_active_at IS NULL OR u.last_active_at < datetime('now', '-30 days'))"
    }

    // 排序欄位（白名單防注入）
    const orderBy =
      sort === 'last_active_at' ? 'u.last_active_at DESC NULLS LAST' : 'u.created_at DESC'
    ...
    LIMIT ? OFFSET ?`
      ...
      .all<
        Omit<User, 'password_hash' | 'google_id'> & {
          rank_id: string | null
...
usersRoutes.get(
  ...
  describeRoute({
    ...
    summary: '獲取用戶統計',
    description:
      '獲取用戶相關統計數據，包括總數、活躍用戶數、各角色統計、本週/本月新增用戶數、認證方式統計等。僅限管理員使用。',
    responses: {
  ...
  async (c) => {
    // 總用戶數
    const totalResult = await c.env.DB.prepare('SELECT COUNT(*) as count FROM users').first<{
      count: number
    }>()
...
usersRoutes.put(
  ...
  describeRoute({
    ...
    summary: '更新用戶角色',
    description:
      '更新指定用戶的角色（user、admin、moderator）。不能更改自己的角色。僅限管理員使用。',
    responses: {
...
usersRoutes.post(
  ...
  describeRoute({
    ...
    summary: '上傳用戶頭像',
    description:
      '上傳用戶頭像圖片到 R2 儲存空間。支援 JPEG、PNG、WebP 格式。會自動刪除舊的頭像並更新用戶資料。',
    responses: {
  ...
  async (c) => {
    ...
    const currentUser = await c.env.DB.prepare('SELECT avatar_url FROM users WHERE id = ?')
      .bind(userId)
