...
aiRoutes.post(
  ...
  async (c) => {
      ...
      if (err instanceof GuardrailError) {
        ...
        c.executionCtx.waitUntil(
          db
            ...
            .bind(crypto.randomUUID(), userId, body.query.slice(0, 500), blockedTrace)
            .run()
            .catch(() => {})
        )
    ...
    const estimatedTokens = Math.ceil(
      (SYSTEM_PROMPT.length + ESTIMATED_CONTEXT_LENGTH + body.query.length) / 2
    )
          ...
          return c.json(
            {
              ...
              message:
                '今日 Token 配額已用盡，明日台灣時間 00:00 重置。充實攀岩日誌可提升等級獲得更多配額。',
              data: baseData,
            },
            429
          )
        ...
        return c.json(
          {
            ...
            message:
              '今日 AI 使用次數已用完，明日台灣時間 00:00 重置。充實攀岩日誌可提升等級獲得更多次數。',
            data: baseData,
          },
          429
        )
...
aiRoutes.post(
  ...
  async (c) => {
    ...
    try {
      ...
      const logRow = await c.env.DB.prepare(`SELECT auto_score FROM ai_query_logs WHERE id = ?`)
        .bind(query_id)
        .first<{ auto_score: number | null }>()
...
aiRoutes.delete(
  ...
  async (c) => {
    ...
    const result = await c.env.DB.prepare(`DELETE FROM chat_sessions WHERE id = ? AND user_id = ?`)
      .bind(sessionId, userId)
      .run()
...
aiRoutes.post(
  ...
  async (c) => {
    ...
    if (role === 'user') {
      ...
      await c.env.DB.prepare(`UPDATE chat_sessions SET updated_at = ? WHERE id = ?`)
        .bind(now, sessionId)
        .run()
    }
  ...
  async (c) => {
    ...
    if (!isAdmin) {
      ...
      const result = await db
        .prepare(
          "UPDATE user_ranks SET daily_ai_used = daily_ai_used + 1, updated_at = datetime('now') WHERE user_id = ? AND daily_ai_used < daily_ai_limit"
        )
        .bind(userId)
        .run()
    ...
    try {
      ...
      if (!isAdmin) {
        await db
          .prepare(
            'UPDATE user_ranks SET daily_ai_used = MAX(0, daily_ai_used - 1) WHERE user_id = ?'
          )
          .bind(userId)
          .run()
      }
