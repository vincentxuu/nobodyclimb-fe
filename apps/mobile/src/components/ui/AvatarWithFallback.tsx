...
import { AVATAR_SIZES, SEMANTIC_COLORS } from '@nobodyclimb/constants'
 ...
 * ```
 */
export function AvatarWithFallback({ src, size = 'md', fallback }: AvatarWithFallbackProps) {
  const avatarSize = AVATAR_SIZES[size]

  // 如果有 fallback 文字，顯示文字縮寫
  const fallbackElement = fallback ? (
    <View style={styles.fallbackContainer}>
      <Text variant="bodyBold" style={[styles.fallbackText, { fontSize: avatarSize * 0.4 }]}>
        {fallback.slice(0, 2).toUpperCase()}
      </Text>
    </View>
  ) : undefined

  return <Avatar source={src ? { uri: src } : undefined} size={size} fallback={fallbackElement} />
}
