...
export function useRouteFilter<T extends Route>(routes: T[]): UseRouteFilterResult<T> {
  ...
  const setFilter = useCallback(
    <K extends keyof RouteFilterOptions>(key: K, value: RouteFilterOptions[K]) => {
  ...
  const filteredRoutes = useMemo(() => {
    ...
    if (filters.grades && filters.grades.length > 0) {
      result = result.filter((route) => filters.grades!.includes(route.grade))
    }

    // 類型過濾
    if (filters.types && filters.types.length > 0) {
      result = result.filter((route) => filters.types!.includes(route.type))
    }

    // 風格過濾
    if (filters.styles && filters.styles.length > 0) {
      result = result.filter((route) => route.style && filters.styles!.includes(route.style))
    }

    // 星級過濾
    if (filters.minStars !== undefined) {
      result = result.filter((route) => (route.stars ?? 0) >= filters.minStars!)
    }
    if (filters.maxStars !== undefined) {
      result = result.filter((route) => (route.stars ?? 0) <= filters.maxStars!)
    }

    // 高度過濾
    if (filters.minHeight !== undefined) {
      result = result.filter((route) => (route.height ?? 0) >= filters.minHeight!)
    }
    if (filters.maxHeight !== undefined) {
      result = result.filter((route) => (route.height ?? 0) <= filters.maxHeight!)
    }
