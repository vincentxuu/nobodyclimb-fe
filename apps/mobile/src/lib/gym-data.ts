 ...
 * 負責讀取和處理岩館 JSON 資料
 *
 * 對應 apps/web/src/lib/gym-data.ts
 */
...
export async function loadGymsData(): Promise<GymsJsonData> {
  ...
  try {
    // 從 Web 端載入 JSON 資料
    // 在生產環境中應該使用 API endpoint
    const response = await fetch('https://nobodyclimb.cc/data/gyms.json')
    if (!response.ok) {
      throw new Error('Failed to fetch gyms data')
    }
    cachedGymsData = await response.json()
    return cachedGymsData as GymsJsonData
  } catch (error) {
 ...
 * 攀岩館類型篩選選項
 */
export const GYM_TYPES = ['所有類型', '上攀', '抱石'] as const
