...
function parseQuestion(block, categoryId, questionNumber) {
  ...
  if (questionMatch) {
    ...
    // 檢查是否為排序題
    if (
      question.question.includes('排列') ||
      question.question.includes('順序') ||
      question.question.includes('依序')
    ) {
  ...
  for (const [categoryId, questions] of Object.entries(allQuestions)) {
    ...
    for (const q of questions) {
      ...
      if (q.referenceSources && q.referenceSources.length > 0) {
        lines.push(
          `      referenceSources: [${q.referenceSources.map((s) => JSON.stringify(s)).join(', ')}],`
        )
      }
