...
const BUCKET_LIST_ITEMS = [
  // 已完成的目標
  {
    ...
    description:
      '這是龍洞的經典路線之一，擁有美麗的橫渡段落和技術性動作。25 米的路線長度需要良好的耐力和技巧。',
    target_grade: '5.11a',
    target_location: '龍洞',
    status: 'completed',
  ...
  // 進行中的目標（帶進度條）
  {
    ...
    category: 'outdoor_route',
    description:
      '挑戰自己在 5.11 這個難度等級的穩定性和全面性，目標是在龍洞和墾丁完成 10 條不同風格的 5.11 路線。',
  ...
  },
  {
    ...
    category: 'training',
    description:
      '克服先鋒墜落恐懼的訓練計畫。透過有計劃的練習墜落，建立對確保系統的信任和克服高度恐懼。',
    target_grade: null,
    target_location: 'Original 岩館',
    status: 'active',
    enable_progress: true,
  ...
  // 進行中的挑戰目標
  {
    ...
    category: 'outdoor_route',
    description:
      '墾丁最經典的 5.12a 路線，擁有連續的 crimps 和需要爆發力的動作。30 米的長度對耐力也是極大考驗。',
    target_grade: '5.12a',
    target_location: '墾丁',
  ...
  },
  {
    ...
    description:
      '挑戰自己參加正式比賽，體驗競技攀岩的氛圍和壓力。目標不是名次，而是享受過程並測試自己在壓力下的表現。',
    target_grade: null,
    target_location: '台灣',
  ...
  // 技能類目標
  {
    ...
    category: 'skill',
    description:
      '掌握校園板（Campus Board）訓練技巧，提升爆發力和指力。這是進階攀岩者必備的訓練方法。',
    target_grade: null,
    target_location: 'Original 岩館',
    status: 'active',
    enable_progress: true,
  ...
  },
  {
    ...
    category: 'skill',
    description:
      '學習傳統攀登中的裂隙技巧，包括 hand jam、foot jam 等技術。這是戶外攀登的重要技能。',
    target_grade: null,
  ...
  },
  {
    ...
    category: 'adventure',
    description:
      '前往抱石聖地 Fontainebleau，攀爬那些傳奇的經典抱石路線，感受抱石運動的歷史和文化。',
    target_grade: null,
  ...
  },
  {
    ...
    category: 'adventure',
    description:
      '結合登山和攀岩，挑戰在高山環境中進行攀岩活動。目標是攀登 3 座百岳並在附近進行攀岩探索。',
    target_grade: null,
  ...
  // 訓練類目標
  {
    ...
    category: 'training',
    description:
      '挑戰自己連續 100 天進行攀岩相關訓練（包括攀爬、肌力訓練、柔軟度訓練等），培養穩定的訓練習慣。',
    target_grade: null,
  ...
  // 受傷恢復類
  {
    ...
    description:
      '2024 年因過度訓練造成右手中指 A2 滑車受傷，目標是完全康復並能回到之前的訓練強度。',
    target_grade: null,
    target_location: null,
    status: 'completed',
    enable_progress: false,
...
async function main() {
  ...
  try {
    ...
    const _beforeStats = getCategoryStats(existingItems)
    ...
    Object.keys(afterStats).forEach((category) => {
      const _stat = afterStats[category]
    })
  } catch (error) {
    console.error('\n✗ 錯誤:', error.message)
    process.exit(1)
