...
function extractKeywords(title, keywordMap) {
  ...
  for (const crag of keywordMap.crags) {
    ...
    if (
      crag.nameEn &&
      crag.nameEn.length > 2 &&
      matchWordBoundary(titleLower, crag.nameEn.toLowerCase())
    ) {
      result.crag = crag
      break
    }
...
async function main() {
  ...
  // 依岩場分組輸出 CSV
  const cragGroups = {}
  for (const [_routeId, data] of Object.entries(routeVideos)) {
  ...
  for (const [cragId, group] of Object.entries(cragGroups)) {
    ...
    for (const data of group.routes) {
      ...
      const videosStr = videos.map((v) => `${v.title}\t${v.url}`).join('\n')
  ...
  if (Object.keys(cragVideos).length > 0) {
    for (const [_cragId, _data] of Object.entries(cragVideos)) {
    }
  }
}
