...
function EditBlogPageContent() {
  ...
  const handleSubmit = async (newStatus: ArticleStatus) => {
    ...
    try {
      ...
      if (response.success) {
        alert(newStatus === 'published' ? t('updateSuccess') : t('draftSuccess'))
        router.push('/profile/articles')
      } else {
        throw new Error(response.message || t('updateFallbackError'))
      }
    } catch (error) {
      console.error('更新文章時出錯:', error)
      const errorMessage = error instanceof Error ? error.message : t('unknownError')
      alert(t('updateError', { error: errorMessage }))
    } finally {
