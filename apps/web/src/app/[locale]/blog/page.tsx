...
const EmptyState = ({
  ...
  searchQuery: string
  category: string
  t: ReturnType<typeof useTranslations<'BlogPage'>>
}) => (
...
const ArticleCard = ({
  article,
  t,
}: ArticleCardProps & { t: ReturnType<typeof useTranslations<'BlogPage'>> }) => {
...
function BlogContent() {
  ...
  const getSelectedCategoryLabel = (): string => {
    if (!selectedCategory) return t('allArticles')
    ...
    <div className="min-h-screen bg-page-content-bg">
      <div className="container mx-auto px-3 py-6 sm:px-4 sm:py-10">
        ...
        <div className="mb-6 sm:mb-8">
          <Breadcrumb items={[{ label: t('home'), href: '/' }, { label: t('blog') }]} />
        </div>
        ...
        {isLoading ? (
          ...
          <EmptyState searchQuery={searchQuery} category={getSelectedCategoryLabel()} t={t} />
        ) : (
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
            ...
            <div className="mb-12 mt-8 flex justify-center sm:mb-16 sm:mt-10">
              {hasMore ? (
                ...
                <p className="text-sm text-wb-70 sm:text-base">{t('allArticlesShown')}</p>
              )}
            </div>
          </>
        )}
...
function BlogPageFallback() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-page-content-bg">
      ...
      <span className="ml-2 text-sm text-wb-70 sm:text-base">{t('loading')}</span>
    </div>
  )
}
