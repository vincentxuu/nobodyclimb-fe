...
export default function AnonymousSharePage() {
  ...
  const handleSubmit = async () => {
    if (stories.length === 0) {
      setError(t('errorNoStory'))
      return
    }

    const sessionId = getSessionId()
    if (!sessionId) {
      setError(t('errorSessionLost'))
      return
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.error || t('errorSubmitFailed'))
      }
    } catch (err: any) {
      setError(err.response?.data?.error || t('errorSubmitFailed'))
    } finally {
      ...
      <div className="fixed bottom-0 left-0 right-0 border-t bg-white p-4">
        <div className="mx-auto flex max-w-2xl items-center justify-between gap-4">
          <div className="text-sm text-gray-500">
            {stats.total > 0 ? (
              <span>{t('filledCount', { count: stats.total })}</span>
            ) : (
          ...
          <div className="flex gap-2">
            ...
            >
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : t('publishAnonymous')}
            </Button>
          </div>
        </div>
      </div>
