  ...
  return (
    <div className="min-h-screen bg-page-content-bg">
      ...
      <div className="container mx-auto px-4 py-6">
        {loading ? (
          <LoadingSpinner text={t('loadingData')} fullPage />
        ) : (
          <>
            {/* 搜尋和篩選 */}
            ...
            {searchQuery && (
              <div className="mb-4 text-sm text-neutral-600">
                {t('searchResultCount', { count: filteredVideos.length })}
                {meta && videoList.length < meta.totalVideos && t('searchResultPartial')}
              </div>
            )}
            ...
            {filteredVideos.length > 0 && (
              <LoadMoreButton
                ...
                hasMore={hasMore}
                text={loadingMore ? t('loadingMore') : t('loadMore')}
                noMoreText={t('allVideosShown')}
              />
            )}
