...
function RouteLayoutContent({
  ...
  // 當 server 端未提供資料時（Worker 間 HTTP 請求限制），改用 client-side hook 取得
  const { data: clientRoutes = [] } = useCragRoutes(cragId)
  const { data: clientAreas = [] } = useCragAreas(cragId)
  const { data: clientCragDetail } = useCragDetail(cragId)

  const routes = serverRoutes.length > 0 ? serverRoutes : clientRoutes
  const areas = serverAreas.length > 0 ? serverAreas : clientAreas
  const cragName = serverCragName || clientCragDetail?.name || ''
