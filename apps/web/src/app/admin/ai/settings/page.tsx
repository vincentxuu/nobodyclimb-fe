      ...
      },
      {
        ...
        fields: [
          ...
          },
          {
            ...
            label: 'Reranker 相關性閾值',
            placeholder: '0.3',
            hint: 'Cross-encoder reranker score 低於此值的文件直接丟棄，減少 LLM context 雜訊（0–1，預設 0.3）',
          },
          {
            key: 'reranker_min_keep',
            label: 'Reranker 最低保留數',
            placeholder: '2',
            hint: '即使全部低於閾值，至少保留 score 最高的前 N 筆文件（1–20，預設 2）',
          },
        ],
      },
      {
        ...
        fields: [
          {
            ...
            label: '信心閾值',
            placeholder: '0.7',
            hint: 'Tool Selection confidence 低於此值時降級為 general_knowledge，避免低信心檢索浪費資源（0–1，預設 0.7）',
          },
        ],
      },
    ],
        ...
        fields: [
          {
            ...
            label: '強警示閾值',
            placeholder: '0.6',
            hint: 'Groundedness 低於此值時，在回答前注入強警示（0–1）',
          },
          {
            key: 'groundedness_disclaimer_mid',
            label: '輕警示閾值',
            placeholder: '0.8',
            hint: 'Groundedness 低於此值時，在回答前注入提醒（應大於強警示閾值）',
          },
          {
        ...
        fields: [
          {
            ...
            label: '啟用 LangGraph 引擎',
            placeholder: '0',
            hint: '0 = 使用原始 Pipeline Engine；1 = 使用 LangGraph 引擎（ai-graph），啟用後 RAG 策略由 LangGraph graph 接管',
            options: [
              { value: '0', label: '停用（Pipeline Engine）' },
              { value: '1', label: '啟用（LangGraph）' },
            ],
          },
        ],
      },
        ...
        fields: [
          {
            ...
            label: '最大子任務數',
            placeholder: '4',
            hint: '規劃階段 LLM 最多生成幾個子任務（1–8），子任務越多搜尋越全面但延遲越高',
          },
          {
            key: 'plan_execute_min_entities',
            label: 'Auto 最低實體數',
            placeholder: '2',
            hint: '僅 auto 模式生效：規劃子任務數少於此值時降級為 agentic（1–5）',
          },
          {
            key: 'planning_timeout_ms',
            label: '規劃超時（ms）',
            placeholder: '8000',
            hint: 'Planning LLM 超時上限，超時則 fallback 到 agentic（3000–15000）',
          },
          {
            key: 'plan_step_timeout_ms',
            label: '子任務超時（ms）',
            placeholder: '5000',
            hint: '每個子任務（embedding + 搜尋）超時上限，超時回傳空結果（2000–10000）',
          },
          {
            key: 'synthesis_timeout_ms',
            label: '合成超時（ms）',
            placeholder: '8000',
            hint: 'Synthesis LLM 超時上限，超時使用 fallback 拼接（3000–15000）',
          },
          {
    ...
    sections: [
      {
        ...
        fields: [
          ...
          },
          {
            ...
            label: 'LLM 生成超時（ms）',
            placeholder: '18000',
            hint: 'LLM 回答生成超時 → 回傳超時錯誤訊息，跳過 evaluation（3000–30000）',
          },
          {
            key: 'hyde_timeout_ms',
            label: 'HyDE 超時（ms）',
...
function TabPanel({ tab, config }: { tab: TabConfig; config: Record<string, string> }) {
  const { mutate: updateConfig, isPending } = useUpdateAIConfig()
