...
export function SelfReflectionTrace({
  ...
  return (
    <div>
      <StageDesc>
        LLM Judge 對初次生成的回答評分後，若品質（Quality &lt; 2/4）或接地性（Groundedness &lt;
        50%）低於閾值，自動觸發重新生成（Regen）。重生成後再次評判，比較兩版本分數，選出品質較佳者作為最終回答。觸發條件：query_type
        = complex 或 pipeline 設定允許。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <div className="space-y-1.5">
            ...
            <p className="text-[10px] text-wb-30">
              觸發重生成條件：Quality &lt; 2（滿分 4）或 Groundedness &lt; 50%
            </p>
            {firstQuality != null && (
              <div className="flex gap-6 pt-0.5">
                <div>
                  ...
                  <p
                    className={`font-bold tabular-nums ${firstQuality >= 3 ? 'text-emerald-600' : firstQuality >= 2 ? 'text-amber-600' : 'text-red-500'}`}
                  >
                    {firstQuality} / 4
                  </p>
                </div>
                {firstGroundedness != null && (
                  <div>
                    ...
                    <p
                      className={`font-bold tabular-nums ${firstGroundedness >= 0.7 ? 'text-emerald-600' : firstGroundedness >= 0.5 ? 'text-amber-600' : 'text-red-500'}`}
                    >
              ...
              <div className="space-y-1.5">
                {regenReason && (
                  <div className="flex items-center gap-2">
                    ...
                    <TraceBadge
                      text={regenReasonLabels[regenReason] ?? regenReason}
                      color="amber"
                    />
                  </div>
                )}
                ...
                {secondQuality != null && (
                  <div className="flex gap-4 pl-5">
                    <div>
                      ...
                      <p
                        className={`font-bold tabular-nums ${secondQuality >= 3 ? 'text-emerald-600' : secondQuality >= 2 ? 'text-amber-600' : 'text-red-500'}`}
                      >
                        {secondQuality} / 4
                      </p>
                    </div>
                    {secondGroundedness != null && (
                      <div>
                        ...
                        <p
                          className={`font-bold tabular-nums ${secondGroundedness >= 0.7 ? 'text-emerald-600' : secondGroundedness >= 0.5 ? 'text-amber-600' : 'text-red-500'}`}
                        >
          ...
          {!triggered ? (
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <TraceBadge
                  text={
                    acceptanceReason === 'regen_accepted'
                      ? '採用重生成答案'
                      : acceptanceReason === 'original_kept'
                        ? '保留原始答案（重生成未改善）'
                        : regenAccepted
                          ? '採用重生成答案'
                          : '保留原始答案（重生成未改善）'
              ...
              {!regenAccepted && secondGroundedness != null && firstGroundedness != null && (
                <p className="text-wb-40 text-[10px]">
                  比較 Groundedness：原始 {(firstGroundedness * 100).toFixed(0)}% vs 重生成{' '}
                  {(secondGroundedness * 100).toFixed(0)}%，保留較高者
                </p>
              )}
            </div>
          ) : (
