...
export function GuardrailsOutputTrace({
  response,
  pipelineStage,
}: {
  ...
  return (
    <div>
      <StageDesc>
        回答送達用戶前的最後安全關卡。偵測系統提示洩漏（System Prompt
        Leakage）、遮蓋個人識別資訊（PII：電話、Email
        等），並在回答超過最大長度時截斷，確保輸出安全合規。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <KVRow
            label="觸發條件"
            value="所有 LLM 回應強制執行（無條件觸發）；洩漏或違規時替換回應內容"
          />
          {response ? (
            <div className="mt-1">
              ...
              <p className="text-wb-40 mt-1">
                原始長度：{go?.original_length ?? response.length} 字元
              </p>
            </div>
          ) : (
        ...
        <StageSection type="decision">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <TraceBadge
                text="System Prompt Leakage"
                color={go?.system_prompt_leaked ? 'red' : 'blue'}
              />
              <span className="text-wb-50">
                {go?.system_prompt_leaked ? '偵測到洩漏，回應已替換' : '無洩漏'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <TraceBadge
                text="PII 過濾"
                color={go?.pii_count && go.pii_count > 0 ? 'amber' : 'blue'}
              />
              <span className="text-wb-50">
                {hasData
                  ? `發現 ${go?.pii_count ?? 0} 筆 PII 並遮蓋`
                  : '移除電話、Email 等個人識別資訊'}
              </span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-1">
            {hasData ? (
              <>
                ...
                {go?.original_length != null &&
                  go?.output_length != null &&
                  go.original_length !== go.output_length && (
