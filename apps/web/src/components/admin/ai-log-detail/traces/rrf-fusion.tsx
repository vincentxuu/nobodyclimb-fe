...
export function RRFFusionTrace({ trace }: { trace: PipelineTrace | null }) {
  ...
  if (!r.rrf)
    return (
      <div>
        <StageDesc>
          將多路搜尋結果以 Reciprocal Rank Fusion 演算法融合：各文件的最終 RRF
          分數為其在各路徑中倒排名的加總，跨路徑去重後依分數門檻過濾低質候選，產出一份統一有序清單。
        </StageDesc>
        <IOFlow>
          <StageSection type="input">
            ...
            <KVRow
              label="各路徑候選"
              value={`${r.candidates_before_filter} 筆（多路徑原始結果）`}
            />
          </StageSection>
  ...
  return (
    <div>
      <StageDesc>
        將多路搜尋結果以 Reciprocal Rank Fusion 演算法融合：各文件的最終 RRF
        分數為其在各路徑中倒排名的加總（score = Σ
        1/(k+rank)，k=60），跨路徑去重後依分數門檻過濾低質候選，產出一份統一有序清單。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
                ...
                {pathEntries.map(([path, count]) => {
                  ...
                  return (
                    <div key={path} className="rounded border border-wb-10 overflow-hidden">
                      <button
                        onClick={() =>
                          hasData ? setExpandedInputPath(isExpanded ? null : path) : undefined
                        }
        ...
        <StageSection type="decision">
          <div className="space-y-1.5">
            <KVRow
              ...
              value={
                <span>
                  RRF score = <span className="font-mono">Σ 1/(60 + rank)</span>（各路徑倒排名加總）
                </span>
              }
            />
              ...
              value={
                <span>
                  <span className="font-mono text-violet-600">
                    {r.rrf.min_score_threshold.toFixed(4)}
                  </span>
            ...
            {rrfResults.length > 0 && (
              <div>
                ...
                >
                  <ChevronUp
                    className={`h-3 w-3 transition-transform ${showMerged ? '' : 'rotate-180'}`}
                  />
                  {showMerged
                    ? '收起'
                    : `展開合併後 ${rrfResults.length} 筆文件（含 RRF 分 + 來源路徑）`}
                </button>
                {showMerged && (
                  <div className="mt-1.5 space-y-0.5">
                    <div className="grid grid-cols-[20px_1fr_56px_auto] gap-x-2 text-[9px] text-wb-30 px-1 pb-0.5 border-b border-wb-8">
                      <span>#</span>
                      <span>文件名稱</span>
                      <span className="text-right">RRF分</span>
                      <span>路徑</span>
                    </div>
                    {rrfResults.map((doc, i) => {
                      ...
                      return (
                        <div
                          key={doc.id}
                          className={`grid grid-cols-[20px_1fr_56px_auto] gap-x-2 items-center text-[10px] px-1 py-0.5 rounded ${passed ? '' : 'opacity-40'}`}
                        >
                          <span className="text-wb-30 tabular-nums">{i + 1}.</span>
                          <span
                            className={`truncate ${passed ? 'text-wb-70' : 'text-wb-40 line-through'}`}
                          >
                            {doc.name ?? doc.id}
                          </span>
                          <span
                            className={`text-right font-mono tabular-nums ${passed ? 'text-violet-600' : 'text-wb-30'}`}
                          >
                            {doc.rrfScore.toFixed(4)}
                          </span>
        ...
        <StageSection type="output">
          <div className="space-y-1">
            ...
            <p className="text-wb-30 text-[10px]">
              已按 RRF 分數降序排列，相同文件出現在越多路徑且排名越前則分數越高
            </p>
            {filtered.length > 0 && (
              <div>
                <button
                ...
                {showOutput && (
                  <div className="mt-1.5 space-y-0.5">
                    ...
                    {filtered.map((doc, i) => (
                      <div
                        key={doc.id}
                        className="grid grid-cols-[20px_1fr_56px_auto] gap-x-2 items-center text-[10px] px-1 py-0.5 rounded hover:bg-wb-3"
                      >
                        <span className="text-wb-40 tabular-nums">{i + 1}.</span>
                        <span className="text-wb-80 truncate">{doc.name ?? doc.id}</span>
                        <span className="text-right font-mono tabular-nums text-emerald-600">
                          {doc.rrfScore.toFixed(4)}
                        </span>
                        <div className="flex gap-0.5 flex-wrap">
                          {doc.paths.map((p) => (
