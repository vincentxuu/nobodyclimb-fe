...
export function QueryParsingTrace({ trace, query }: { trace: PipelineTrace; query: string }) {
  ...
  return (
    <div>
      <StageDesc>
        使用 LLM 分析查詢意圖，決定呼叫哪個搜尋工具（路線搜尋 / 岩場搜尋 /
        通識問答），同時抽取結構化過濾條件（地區、難度、路線類型等）。是後續 Metadata Filter
        建構與搜尋策略（HyDE、Multi-Query、Agentic）選擇的依據。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <KVRow
            label="觸發條件"
            value="所有非快取查詢必經；輸出 tool / query_type / params 決定後續 Pipeline 路徑"
          />
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] tabular-nums font-medium">
                      {(ts.confidence * 100).toFixed(0)}%
                    </span>
                  </div>
                  {ts.alternative && (
                    <span className="text-[10px] text-wb-40">
                      備選：
                      <TraceBadge text={ts.alternative} color="default" />
                    </span>
                  )}
                </div>
              )}
              {qp.retrieval_method && qp.retrieval_method !== 'hybrid' && (
                <div className="flex items-center gap-2">
                  ...
                  <TraceBadge
                    text={
                      qp.retrieval_method === 'bm25'
                        ? 'BM25（精確匹配）'
                        : qp.retrieval_method === 'vector'
                          ? 'Vector（語意搜尋）'
                          : qp.retrieval_method
                    }
              ...
              {ts?.fallback?.triggered && (
                <div className="flex items-center gap-2 mt-1 rounded border border-amber-200 bg-amber-50/60 px-2 py-1">
                  ...
                  <span className="text-[10px] text-wb-60">
                    {ts.fallback.from_tool} → {ts.fallback.to_tool}（{ts.fallback.reason}）
                  </span>
                </div>
              )}
            </div>
