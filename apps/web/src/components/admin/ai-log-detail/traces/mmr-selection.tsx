...
export function MMRSelectionTrace({
  trace,
  sources,
}: {
  ...
  return (
    <div>
      <StageDesc>
        從 Reranking 後的候選中，以 Maximal Marginal
        Relevance（MMR）迭代選出兼顧相關性與多樣性的文件組合：每輪選取「與查詢最相關，同時與已選集合相似度最低」的文件。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <div className="space-y-1.5">
            <KVRow label="觸發條件" value="所有向量搜尋路徑必經；Reranking 後執行多樣性選取" />
            {trace?.retrieval?.reranker?.top_scores &&
            trace.retrieval.reranker.top_scores.length > 0 ? (
              <div>
                <p className="text-wb-40 text-[10px] mb-1">
                  輸入文件（Reranker 輸出，{m.input_count} 筆）：
                </p>
                <div className="space-y-0.5">
                  {trace.retrieval.reranker.top_scores.map((doc, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-[10px] px-1">
                      ...
                      <span
                        className={`font-mono tabular-nums shrink-0 ${doc.score >= 0.5 ? 'text-emerald-600' : doc.score >= 0.2 ? 'text-amber-600' : 'text-wb-40'}`}
                      >
                        {doc.score.toFixed(3)}
                      </span>
                    </div>
