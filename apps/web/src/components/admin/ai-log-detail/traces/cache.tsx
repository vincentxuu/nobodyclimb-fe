...
export function CacheTrace({
  ...
  return (
    <div>
      <StageDesc>
        在執行完整 RAG Pipeline 之前先查詢快取，避免相同查詢重複運算。支援兩種命中模式：KV
        精確快取（完全相同的查詢鍵）與語義相似度快取（向量餘弦相似度超過閾值的近似查詢）。命中時直接回傳結果，跳過後續所有
        Pipeline 階段。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <KVRow
            label="觸發命中條件"
            value="KV 精確命中：Cache Key 完全相符 ／ 語義命中：向量餘弦相似度 ≥ 閾值"
          />
        ...
        <StageSection type="decision">
          <div className="flex items-center gap-2">
            ...
            {hit === true && cacheType === 'semantic' && (
              <TraceBadge text="語義相似命中（向量）" color="violet" />
            )}
          </div>
        </StageSection>
