...
export function PopularityRerankTrace({ trace, sources }: { trace: PipelineTrace | null; sources: Array<{ title?: string; type?: string; score?: number }> }) {
          ...
          <div className="space-y-1.5">
            <KVRow label="熱門度加權" value={
              <span>
                ...
                <span className="ml-1.5 text-wb-30 text-[10px]">依影片數正規化熱門度（0–1）的加成係數</span>
              </span>
            } />
            ...
            {pr.top_selected && pr.top_selected.length > 0 && (
              <div>
                ...
                <div className="grid text-[10px] text-wb-30 mb-1 px-2" style={{ gridTemplateColumns: '1.2rem 1fr 3rem 3rem 3rem' }}>
                  ...
                  <span className="text-right" title="Reranker 相關性分">相關性↑</span>
                  <span className="text-right" title="影片數正規化熱門度">熱門度↑</span>
                  <span className="text-right" title="加權後最終分">最終分↑</span>
                </div>
