...
export function FilterTrace({
  ...
  if (!f)
    return (
      <div>
        <StageDesc>
          將 query_parsing 抽取的結構化 params 轉換為 Vectorize 向量資料庫的 Metadata
          Filter，在向量搜尋時限縮候選範圍。支援 LLM 解析（llm_parsed）、Regex
          降級（regex_fallback）、相似路線（sim_route）與對話歷史補充（history_supplemented）等來源。
        </StageDesc>
        <IOFlow>
      ...
      <IOFlow>
        <StageSection type="input">
          <div className="space-y-1">
            <KVRow
              label="觸發條件"
              value="tool = search_routes / search_crags 且 query_parsing 抽取到結構化 params；通識問答跳過"
            />
        ...
        <StageSection type="decision">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              {f.source === 'regex_fallback' && (
                <span className="text-wb-50">LLM 解析失敗，降級為 Regex</span>
              )}
            </div>
