...
export function AgenticTrace({ trace }: { trace: PipelineTrace }) {
  ...
  return (
    <div>
      <StageDesc>
        多步驟 Agentic RAG 模式。LLM 自主規劃多輪搜尋：每步驟由 LLM 決策下一動作（RETRIEVE 繼續搜尋
        / BROADEN 放寬條件 / SWITCH_TOOL 切換工具 / DECOMPOSE 拆分子查詢 / VERIFY 交叉驗證 / ANSWER
        已足夠回答），動態調整查詢直到累積足夠高品質文件或達到最大步數上限。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <div className="space-y-1">
            ...
            <KVRow
              label="策略"
              value={<TraceBadge text="Agentic Multi-Step RAG" color="violet" />}
            />
              ...
              {ini.quality_check && (
                <>
                  ...
                  <KVRow
                    ...
                    value={
                      <span
                        className={ini.quality_check.passed ? 'text-emerald-600' : 'text-amber-600'}
                      >
                ...
                {a.steps.map((s, i) => (
                  <li key={i} className="space-y-0.5">
                    <div className="flex items-start gap-2">
                      ...
                      {s.refinedQuery && (
                        <span className="text-wb-60 italic text-[11px] line-clamp-1">
                          {s.refinedQuery}
                        </span>
                      )}
                      {s.type === 'SWITCH_TOOL' && s.targetTool && (
                        <span className="text-wb-60 text-[11px]">
                          → <TraceBadge text={s.targetTool} color="blue" />
                        </span>
                      )}
                      ...
                      {s.docs_retrieved != null && (
                        <span className="ml-auto shrink-0 text-[10px] text-wb-40 tabular-nums">
                          {s.docs_retrieved} 筆
                        </span>
                      )}
                    </div>
          ...
          <div className="space-y-2">
            <div className="flex gap-4">
              <div>
                ...
                <p className="text-base font-bold text-wb-90 tabular-nums">
                  {a.final_doc_count} 筆
                </p>
              </div>
              <div>
            ...
            {a.termination_reason && (
              <div className="flex items-center gap-2">
                ...
                <TraceBadge
                  ...
                  color={
                    a.termination_reason === 'enough_docs'
                      ? 'emerald'
                      : a.termination_reason === 'no_improvement'
                        ? 'amber'
                        : 'default'
                  }
                />
