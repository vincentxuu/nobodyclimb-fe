...
export function PlanAndExecuteTrace({ trace }: { trace: PipelineTrace }) {
  ...
  if (hasFallback) {
    ...
    return (
      <div>
        <StageDesc>
          Plan-and-Execute：LLM 規劃子任務 → 獨立搜尋 → 合成統一 context。本次規劃失敗，已降級至
          Agentic 模式。
        </StageDesc>
        <IOFlow>
  ...
  return (
    <div>
      <StageDesc>
        Plan-and-Execute：LLM 將查詢拆解為多個子任務計畫 → 各子任務獨立搜尋（embedding + BM25 +
        RRF）→ LLM 合成為結構化 context。適合涉及多個岩場或路線的比較型查詢。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
            ...
            {pe.plan && (
              <>
                <KVRow
                  label="執行模式"
                  value={
                    pe.plan.execution_mode === 'parallel'
                      ? '並行'
                      : pe.plan.execution_mode === 'sequential'
                        ? '循序'
              ...
              <ol className="space-y-1.5">
                {pe.steps.map((s, i) => (
                  <li key={i} className="flex items-start gap-2">
                    ...
                    <span className="text-wb-60 italic text-[11px] line-clamp-1 flex-1">
                      {s.query}
                    </span>
                    <span className="shrink-0 text-[10px] text-wb-40 tabular-nums">
                      {s.result_count} 筆 / {s.duration_ms} ms
                    </span>
            ...
            <div className="mt-2 rounded border border-amber-200 bg-amber-50/50 px-2.5 py-2 space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-wb-50">
                  觸發步驟 #{pe.adaptive_replan_info.trigger_step_id}（
