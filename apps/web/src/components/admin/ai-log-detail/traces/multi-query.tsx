...
export function MultiQueryTrace({ trace, query }: { trace: PipelineTrace; query: string }) {
  ...
  return (
    <div>
      <StageDesc>
        使用 LLM 將原始查詢改寫為多個語義不同但意圖相同的子查詢，各子查詢分別在 retrieval
        階段執行獨立的向量搜尋。透過多角度表述提升向量召回率，最終在 RRF
        合併時整合各路徑結果。觸發條件：query_type = complex 且配置允許。
      </StageDesc>
      <IOFlow>
        <StageSection type="input">
          <div className="space-y-1">
            ...
            <div className="space-y-0.5">
              ...
              <p className="font-mono text-[11px] text-wb-70 bg-wb-5 rounded px-2 py-1.5 break-all">
                {query}
              </p>
            </div>
          </div>
