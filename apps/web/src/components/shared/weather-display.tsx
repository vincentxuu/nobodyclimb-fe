...
type SharedUITranslator = ReturnType<typeof useTranslations<'SharedUI'>>
...
// 星期常數（使用翻譯 key，依 date.getDay() 索引）
const WEEKDAY_KEYS = [
  'weekdaySun',
  'weekdayMon',
  'weekdayTue',
...
function formatForecastDate(
  ...
  todayStr: string,
  tomorrowStr: string,
  t: SharedUITranslator
): string {
  const date = new Date(dateString)
  // 根據實際時間判斷早晚：06:00 為早，18:00 為晚
  const hour = date.getHours()
  const period = hour < 12 ? t('forecastPeriodMorning') : t('forecastPeriodEvening')

  let dayLabel: string
  if (date.toDateString() === todayStr) {
    dayLabel = t('forecastToday')
  } else if (date.toDateString() === tomorrowStr) {
    dayLabel = t('forecastTomorrow')
  } else {
    dayLabel = t(WEEKDAY_KEYS[date.getDay()])
  }
...
function formatCondition(
  ...
  if (!condition) return t('weatherConditionUnknown')
...
export function WeatherDisplay({
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          setError(response.message || t('weatherFetchFailed'))
        }
      } catch (err) {
        console.error('Failed to fetch weather:', err)
        setError(t('weatherServiceUnavailable'))
      } finally {
        setLoading(false)
      }
    }
    ...
    fetchWeather()
  }, [location, latitude, longitude, t])

  if (loading) {
    return (
      <div className={`flex items-center justify-center p-4 ${className}`}>
        <Loader2 className="h-5 w-5 animate-spin text-gray-400" />
        <span className="ml-2 text-sm text-gray-500">{t('weatherLoading')}</span>
      </div>
    )
  }
  ...
  if (error || !weather) {
    return (
      <div className={`flex items-center p-4 text-gray-500 ${className}`}>
        ...
        <span className="ml-2 text-sm">{error || t('weatherFetchFailed')}</span>
      </div>
    )
  }
    ...
    return (
      ...
      >
        ...
        <div className="border-l border-gray-300 pl-2">
          <span className="text-sm text-gray-600">{formatCondition(weather.condition, t)}</span>
    ...
    <div className={`rounded-lg bg-gray-100 p-6 ${className}`}>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="font-medium text-gray-700">{formatCondition(weather.condition, t)}</p>
      ...
      {showForecast && weather.forecast.length > 0 && (
        <div className="border-t border-gray-200 pt-4">
          <h4 className="mb-3 text-sm font-medium text-gray-700">{t('weatherForecastTitle')}</h4>
            ...
            {weather.forecast.slice(0, FORECAST_PERIODS).map((day, index) => (
              <div key={`${day.date}-${index}`} className="rounded-lg bg-white p-2 text-center">
                <p className="text-xs font-medium text-gray-600">
                  {formatForecastDate(day.date, todayStr, tomorrowStr, t)}
                </p>
                <p className={`my-1 text-xs ${getWeatherColor(day.condition)}`}>
                  {formatCondition(day.condition, t, 4)}
                </p>
