  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50 p-4">
      ...
      >
        ...
        <h1 className="mb-2 text-xl font-bold">{t('completeTitle')}</h1>
        <p className="mb-2 text-gray-600">{t('completeDescription')}</p>
        <p className="mb-2 text-lg font-medium text-[#1B1A1A]">{anonymousName}</p>
        <p className="mb-6 text-sm text-gray-500">{t('storyCount', { count: totalStories })}</p>

        <div className="mb-6 rounded-lg bg-yellow-50 p-4 text-left">
          <div className="flex items-start gap-2">
            <Sparkles className="mt-0.5 h-5 w-5 text-yellow-600" />
            <div>
              <p className="font-medium text-yellow-800">{t('claimTitle')}</p>
              <p className="mt-1 text-sm text-yellow-700">{t('claimDescription')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <Link href={`/biography/${slug}`}>
            <Button className="w-full">{t('viewMyStory')}</Button>
          </Link>
          <Link href="/biography">
