...
import { Youtube, Plus, LogIn, ExternalLink } from 'lucide-react'
...
export function RouteYouTubeSection({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true)
      try {
        const res = await getRouteQuickShareYouTube(routeId, { limit: 20 })
        if (isMounted) {
          ...
          <div className="space-y-4">
            {allVideos.map((video, index) => (
              ...
              >
                ...
                {/* 用戶分享的影片顯示資訊 */}
                {video.source === 'user' && (video.caption || video.username) && (
                  <div className="mt-2 flex items-center justify-between text-sm">
                    {video.caption && (
                      <p className="text-gray-600">{video.caption}</p>
                    )}
                    <div className="flex items-center gap-2 text-gray-400">
                      <span>by {video.displayName || video.username}</span>
                      <a
                        href={video.url}
                        target="_blank"
