      ...
      {isVisible && (
        ...
        >
          ...
          <div className="mb-4 flex items-center gap-3">
            {isCorrect ? (
              <>
                ...
                <span className="text-xl font-bold text-[#22C55E]">{t('correctTitle')}</span>
              </>
            ) : (
              <>
                <XCircle className="h-8 w-8 text-[#EF4444]" />
                <span className="text-xl font-bold text-[#EF4444]">{t('wrongTitle')}</span>
              </>
            )}
          </div>
          ...
          {referenceSources && referenceSources.length > 0 && (
            <div className="mb-4">
              <div className="mb-1 text-sm font-medium text-[#535353]">{t('referenceLabel')}</div>
          ...
          <Button onClick={onContinue} className="w-full" size="lg">
            {isCorrect ? t('nextQuestion') : t('continue')} →
          </Button>
        </motion.div>
      )}
    </AnimatePresence>
